...
class FileTest extends TestHelper {
  ...
  private def check(ast: TranslationUnit, featureModel: FeatureModel): Boolean = new CTypeSystemFrontend(ast, featureModel).checkAST().forall(_.isWarning)

  @unused
  private def d(n: String) = FeatureExprFactory.createDefinedExternal(n)

  //async.i
  @Ignore def test1(): Unit = {
    assert(check("test1.xi"))
  }

  @Test def busybox_ar(): Unit = {
    assert(check("ar.xi"))
  }

  @Test def boa_boa(): Unit = {
    assert(check("boa.xi"))
  }

  @Test def boa_boa_pi(): Unit = {
    assert(check("boa.pi"))
  }

  @Test def busybox_top_pi(): Unit = {
    assert(check("top.pi"))
  }

  @Test def busybox_umount_pi(): Unit = {
    assert(check("umount.pi"))
  }

  @Test def busybox_udf_pi(): Unit = {
    assert(check("udf.pi"))
  }

  @Ignore("not finished yet")
  @Test def linux_fork_pi(): Unit = {
    assert(check("fork_.pi" /*, FeatureModel.create(linux_fork_fm)*/))
  }

  @Test def toybox_patch_pi(): Unit = {
    assert(check("patch.pi"))
  }

  @Test def toybox_netcat_pi(): Unit = {
    assert(check("netcat.pi"))
  }

  @Test def busybox_modutils_pi(): Unit = {
    assert(check("modutils-24.pi"))
  }

  @Test def busybox_smemcap_pi(): Unit = {
