...
class StructTest extends AnyFunSuite with CEnv with Matchers with TestHelperTS {


  test("StructEnv behavior") {
    var env = new StructEnv()
    env.isComplete("a", isUnion = true) should be(False)

    //struct a;
    env = env.addIncomplete(Id("a"), isUnion = true, True, 1)
    env.isComplete("a", isUnion = true) should be(False)

    //struct a {double x;} //same scope should make it complete
    env = env.addComplete(Id("a"), isUnion = true, True, new ConditionalTypeMap() + ("x", True, null, One(CDouble())), 1)
    env.isComplete("a", isUnion = true) should be(True)
    ...
    env = env.addIncomplete(Id("a"), isUnion = true, fa, 2)
    env.isComplete("a", isUnion = true) should be(fa.not())

    //struct a{double x; double y;};
    env = env.addComplete(Id("a"), isUnion = true, fa, new ConditionalTypeMap() + ("x", True, null, One(CDouble())) + ("y", True, null, One(CDouble())), 2)
    env.isComplete("a", isUnion = true) should be(True)

    val fields = env.getFieldsMerged("a", isUnion = true)
