...
trait AbstractGenerator {
  ...
  case class Config(vals: List[Int]) {
    override def toString: String = vals.mkString("conf", "_", "")

    def updated(idx: Int, v: Int): Config = Config(vals.updated(idx, v))
  }
  ...
  def generate(className: String, configs: Iterator[Config]): Unit = {
  ...
  def writeTest(@unused c: Config, testBody: String, msg: String, exitcode: Int): String = {
    var result = ""
    if (msg.nonEmpty)
      result += "        /* gcc reports:\n" +
        msg + "\n        */\n"
    ...
    val isWarning = msg contains "warning: "
