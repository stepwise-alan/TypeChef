...
class CertSecurityTest extends AnyFunSuite with Matchers with TestHelper {
  ...
  private def check(ast: TranslationUnit, enableAnalysis: Boolean): Boolean = {
    ...
    ).makeSilent().checkAST(ignoreWarnings = false).forall(_.severity == Severity.Warning)
  }
  ...
  test("DCL31-C. Declare identifiers before using them -- implicit function declaration") {
    ...
    assert(!check(
      """
        ...
        |  return a;
        |}
            """.stripMargin, enableAnalysis = true))
  }
