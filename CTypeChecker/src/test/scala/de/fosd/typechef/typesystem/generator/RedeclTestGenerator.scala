...
object RedeclTestGenerator extends App with AbstractGenerator {
  ...
  def _firstDeclKind(c: Config): Int = c.vals.head
  ...
  override def genTest(c: Config): List[String] = {
    ...
    val first = genDecl(_firstDeclKind(c), _firstReturnType(c), _firstParamType(c), extraParam = false, renamedParam = false)
