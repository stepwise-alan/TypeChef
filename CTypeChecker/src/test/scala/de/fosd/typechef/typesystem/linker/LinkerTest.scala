...
class LinkerTest extends AnyFunSuite with Matchers with TestHelper {

  val tfun: CFunction = CFunction(Seq(), CVoid()) // () -> void
  val tfun2: CFunction = CFunction(Seq(CFloat()), CVoid()) // float -> void

  val ffoo: CSignature = CSignature("foo", tfun, True, Seq())
  val ffoo2: CSignature = CSignature("foo", tfun2, True, Seq())
  val fbar: CSignature = CSignature("bar", tfun, True, Seq())
