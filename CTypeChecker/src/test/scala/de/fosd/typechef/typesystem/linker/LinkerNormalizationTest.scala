...
class LinkerNormalizationTest extends AnyFunSuite with Matchers with TestHelper {
  ...
  val strictnessValues: Set[Strictness] = Set(LINK_NAMEONLY, LINK_RELAXED, LINK_STRICT)

  private def assertEqualAfterNormalization(a: CType, b: CType, strictness: Strictness = LINK_STRICT): Unit = assertEquality(a, b, expectedEquality = true, strictness)

  private def assertUnequalAfterNormalization(a: CType, b: CType, strictness: Strictness = LINK_STRICT): Unit = assertEquality(a, b, expectedEquality = false, strictness)

  private def assertEquality(a: CType, b: CType, expectedEquality: Boolean, strictness: Strictness): Unit = {
    ...
    for (s <- strictnessValues) {
      ...
      assert(_linked.isComplete(s) == expectedResult, _linked.toString + (if (expectedResult) " is not complete" else " is complete") + " with " + s)
    }
  }
  ...
  test("equal after normalization only with relaxed") {
    ...
    assertEqualAfterNormalization(
      CFunction(Seq(CPointer(CStruct("foo"))), CVoid()),
