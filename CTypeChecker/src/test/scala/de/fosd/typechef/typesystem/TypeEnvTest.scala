...
class TypeEnvTest extends AnyFunSuite with Matchers with CTypeSystem with CEnvCache with CTypeCache with TestHelper {

  import de.fosd.typechef.typesystem.CType.makeCType

  val _l: One[CType] = OneT(CSigned(CLong()).toCType)
  val _i: One[CType] = OneT(CSigned(CInt()).toCType)
  val _ui: One[CType] = OneT(CUnsigned(CInt()).toCType)
  val x_i: Choice[CType] = Choice(fx, _i, OneT(CUndefined.toCType))
  ...
  val lastDecl: ExternalDef = ast.defs.last.entry
  ...
  test("parse struct decl") {
    ...
    //struct should be in environment
    env.isComplete("account", isUnion = false) should be(True)
    env.isComplete("account", isUnion = true) should be(False) //not a union

    env.isComplete("uaccount", isUnion = false) should be(False)
    env.isComplete("uaccount", isUnion = true) should be(True) //a union

    env.isComplete("announcedStruct", isUnion = false) should be(False) //announced structs should be in the environment, but empty
    env.getFieldsMerged("announcedStruct", isUnion = false) should be(Symbol("isEmpty"))

    val accountStruct = env.getFieldsMerged("account", isUnion = false)
  ...
  test("variable environment") {
    ...
    env("ua") should be(OneT(CPointer(CStruct("uaccount", isUnion = true))))
  ...
  def t(): Unit = {
    ...
    val lastStmt = fundef.stmt.innerStatements.last.entry
  ...
  test("nested functions (lexical) scoping") {
    ...
    val fundef = ast.defs.takeRight(3).head.entry.asInstanceOf[FunctionDef]
    val nestedFundef = fundef.stmt.innerStatements.tail.head.entry.asInstanceOf[NestedFunctionDef]
  ...
  test("typedef synonyms") {
    ...
    //structure definitions should be recognized despite typedefs
    val structenv: StructEnv = lookupEnv(ast.defs.last.entry).structEnv
    structenv.isComplete("pair", isUnion = false) should be(True)
    structenv.isComplete("mystr", isUnion = false) should be(False)
    ...
    val ast = compileCode(
      """
            ...
            mymyint inner;
                                """)
    // val typedefs =
    lookupEnv(ast.defs.last.entry).typedefEnv
    // println(typedefs)
  ...
  test("anonymous struct and typedef") {
    ...
    val env = lookupEnv(fundef.stmt.innerStatements.last.entry).varEnv
  ...
  test("nested anonymous structs nested") {
    ...
    //        println(env2)
    val fields = env2.getFieldsMerged("xx", isUnion = false)
  ...
  test("conditional structs") {
    ...
    structenv.isComplete("s1", isUnion = false) should be(True)
    structenv.isComplete("s2", isUnion = false) should be(fx)
    structenv.isComplete("s3", isUnion = false) should be(fx or fy)

    val structS1 = structenv.getFieldsMerged("s1", isUnion = false)
    ...
    val structS2 = structenv.getFieldsMerged("s2", isUnion = false)
    structS2("a") should be(Choice(fx, _i, OneT(CUndefined)))
    structS2("b") should be(Choice(fx and fy, _i, OneT(CUndefined)))

    val structS3 = structenv.getFieldsMerged("s3", isUnion = false)
    ...
    venv("vs1") should be(OneT(CStruct("s1")))
    venv("vvs1") should be(OneT(CStruct("s1")))
    venv("vs2") should be(Choice(fx, OneT(CStruct("s2")), OneT(CUndefined)))
    venv("vvs2") should be(Choice(fx, OneT(CStruct("s2")), OneT(CUndefined)))
    ConditionalLib.equals(
      venv("vs3"),
      Choice(fx or fy, OneT(CStruct("s3")), OneT(CUndefined))) should be(true)
    ConditionalLib.equals(
      venv("vvs3"),
      Choice(fx or fy, OneT(CStruct("s3")), OneT(CUndefined))) should be(true)
  ...
  test("conditional variable environment") {
    ...
    val fundef = ast.defs.takeRight(3).head.entry.asInstanceOf[FunctionDef]
    val fenv = lookupEnv(fundef.stmt.innerStatements.last.entry).varEnv
      ...
      """
            void foo() {
                ...
                m->next->dir;
                               ...
                               """)
    val fundef = ast.defs.last.entry.asInstanceOf[FunctionDef]
    val exprStmt = fundef.stmt.innerStatements.last.entry.asInstanceOf[ExprStatement]

    val env = lookupEnv(exprStmt.expr).varEnv
    env("m") should be(OneT(CPointer(CStruct("mtab_list"))))

    val senv = lookupEnv(exprStmt.expr).structEnv
    senv.getFieldsMerged("mtab_list", isUnion = false) should not be null
  ...
  test("nested structs") {
    ...
    val env = lookupEnv(exprStmt.expr).varEnv
    env("m") should be(OneT(CPointer(CStruct("volume_descriptor"))))

    val senv = lookupEnv(exprStmt.expr).structEnv
    senv.getFieldsMerged("volume_descriptor", isUnion = false) should not be null
    //        println(senv.getFieldsMerged("volume_descriptor", false))
    senv.getFieldsMerged("descriptor_tag", isUnion = false) should not be null
    //        println(senv.getFieldsMerged("descriptor_tag", false))
    senv.getFieldsMerged("X", isUnion = false) should not be null
    //        println(senv.getFieldsMerged("X", false))
    senv.getFieldsMerged("Y", isUnion = false) should not be null
