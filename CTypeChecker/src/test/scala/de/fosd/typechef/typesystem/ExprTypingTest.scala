...
class ExprTypingTest extends AnyFunSuite with CTypeSystem with CEnv with Matchers with TestHelper {
  ...
  protected def assertCondEquals(exp: Conditional[CType], act: Conditional[CType]): Unit = {
  ...
  val astructEnv: StructEnv =
    new StructEnv().addComplete(
      Id("str"), isUnion = false, True, new ConditionalTypeMap() + ("a", True, null, One(CDouble())) + ("b", True, null, One(CStruct("str"))), 1).addComplete(
      Id("vstrA"), isUnion = false, fx, new ConditionalTypeMap() + ("a", fx and fy, null, _l) + ("b", fx, null, One(CStruct("str"))), 1).addComplete(
      Id("vstrB"), isUnion = false, True, new ConditionalTypeMap() + ("a", True, null, _i) + ("b", True, null, _i) + ("c", fx.not(), null, _i), 1
    )
  ...
  test("conditional primitives and pointers") {
    ...
    exprV("&c") should be(c_i_l.map(_.map(CPointer).toObj))
