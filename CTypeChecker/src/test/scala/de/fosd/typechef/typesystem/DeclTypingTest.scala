...
class DeclTypingTest extends AnyFunSuite with CTypeSystem with Matchers with TestHelper {
  ...
  private def declCT(code: String): Conditional[AType] = declTL(code).head._2.map(_.atype)
  ...
  test("struct declarations") {
    ...
    declT(
      """struct mtab_list {
                ...
                } *mtl;""") should be(CPointer(CStruct("mtab_list")))

  }
  test("union declarations") {
    declT("union { double a;} foo;").isInstanceOf[CAnonymousStruct] should be(true)
    declT("union a foo;") should be(CStruct("a", isUnion = true))
    declT("union a { double a;} foo;") should be(CStruct("a", isUnion = true))
  ...
  val ui: CType = CUnsigned(CInt()).toCType
  test("attributes") {
