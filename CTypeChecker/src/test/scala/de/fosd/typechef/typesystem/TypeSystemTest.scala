...
class TypeSystemTest extends AnyFunSuite with Matchers with TestHelperTS {
...
return 1;
  ...
  ignore("check label environment") {
    //TODO check label environment
    assertResult(false) {
      check(
        """
         void foo() {&&__lab;}
