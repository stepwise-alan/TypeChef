...
class CTypesTest extends AnyFunSuite with CTypeSystem with Matchers {
  ...
  test("wellformed types") {
    ...
    ).view.mapValues(x => One(StructTag(isComplete = true, x, 0))).toMap)
    ...
    wf(CStruct("wf1"))
    nwf(CStruct("wf1", isUnion = true)) //union not allowed as struct and vice versa
    wf(CStruct("wf2", isUnion = true))
    nwf(CStruct("wf2"))
    wf(CStruct("wf3"))
    wf(CStruct("wf4"))
    //        nwf(CStruct("nwf1"))
