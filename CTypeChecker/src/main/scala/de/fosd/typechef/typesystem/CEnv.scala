...
import _root_.de.fosd.typechef.featureexpr.FeatureExpr
import _root_.de.fosd.typechef.parser.c.{AST, Id}
...
trait CEnv {
  ...
  protected class Env(
    ...
    //varenv
    def updateVarEnv(newVarEnv: VarTypingContext): Env = if (newVarEnv == varEnv) this else copy(varEnv = newVarEnv)

    def addVar(name: String, f: FeatureExpr, d: AST, t: Conditional[CType], kind: DeclarationKind, scope: Int, linkage: Conditional[Linkage]): Env = updateVarEnv(varEnv + (name, f, d, t, kind, scope, linkage))

    def addVars(vars: Seq[(String, FeatureExpr, AST, Conditional[CType], DeclarationKind, Linkage)], scope: Int): Env =
      updateVarEnv(vars.foldLeft(varEnv)((ve, v) => ve.+(v._1, v._2, v._3, v._4, v._5, scope, v._6)))

    def addVars2(vars: Seq[(String, FeatureExpr, AST, Conditional[CType], DeclarationKind, Conditional[Linkage])], scope: Int): Env =
      updateVarEnv(vars.foldLeft(varEnv)((ve, v) => ve.+(v._1, v._2, v._3, v._4, v._5, scope, v._6)))

    def addVars(vars: Seq[(String, FeatureExpr, AST, Conditional[CType])], kind: DeclarationKind, scope: Int, linkage: Linkage): Env =
      updateVarEnv(vars.foldLeft(varEnv)((ve, v) => ve.+(v._1, v._2, v._3, v._4, kind, scope, linkage)))

    //structenv
    def updateStructEnv(s: StructEnv): Env = if (s == structEnv) this else copy(structEnv = s)

    //enumenv
    def updateEnumEnv(s: EnumEnv): Env = if (s == enumEnv) this else copy(enumEnv = s)

    //enumenv
    def updateLabelEnv(s: LabelEnv): Env = if (s == labelEnv) this else copy(labelEnv = s)
    ...
    def addTypedefs(typedefs: ConditionalTypeMap): Env = updateTypedefEnv(typedefEnv ++ typedefs)

    def addTypedefs(typedefs: Seq[(String, FeatureExpr, (AST, Conditional[CType]))]): Env = updateTypedefEnv(typedefEnv ++ typedefs)

    def addTypedef(name: String, f: FeatureExpr, d: AST, t: Conditional[CType]): Env = updateTypedefEnv(typedefEnv + (name, f, d, t))

    //expectedReturnType
    def setExpectedReturnType(newExpectedReturnType: Conditional[CType]): Env = this.copy(expectedReturnType = Some(newExpectedReturnType))

    def incScope(): Env = copy(scope = this.scope + 1)

    def markDead(condition: FeatureExpr): Env = this.copy(isDeadCode = this.isDeadCode or condition)

    def addCompletenessCheck(check: Env => Unit): Env = this.copy(openCompletenessChecks = check :: this.openCompletenessChecks)

    def forceOpenCompletenessChecks(): Unit = {
      openCompletenessChecks.foreach(_ (this))
    }

    def markSecurityRelevant(reason: String): Env = this.copy(securityRelevantLocation = Some(reason))

    def markSecurityRelevant(mark: Boolean, reason: String): Env = if (mark) this.copy(securityRelevantLocation = Some(reason)) else this
  ...
  class StructEnv(private val env: Map[(String, Boolean), Conditional[StructTag]]) {
    ...
    private val incompleteTag = StructTag(isComplete = false, emptyFields, -1)
    ...
    def isCompleteUnion(name: String): FeatureExpr = isComplete(name, isUnion = true)

    def isCompleteStruct(name: String): FeatureExpr = isComplete(name, isUnion = false)

    def addIncomplete(id: Id, isUnion: Boolean, condition: FeatureExpr, scope: Int): StructEnv = {
      //overwrites complete tags in lower scopes, but has no effects otherwise
      val name = id.name
      val key = (name, isUnion)
      val newTag = StructTag(isComplete = false, emptyFields, scope, Some(id))
    ...
    def addComplete(id: Id, isUnion: Boolean, condition: FeatureExpr, fields: ConditionalTypeMap, scope: Int): StructEnv = {
      // always override previous results, check elsewhere that not replace incorrectly
      val name = id.name
      val key = (name, isUnion)
      val prevTag: Conditional[StructTag] = env.getOrElse(key, One(incompleteTag))
      val result: Conditional[StructTag] = Choice(condition, One(StructTag(isComplete = true, fields, scope, Some(id))), prevTag).simplify
      new StructEnv(env + (key -> result))
     ...
     */
    def whenHasDefinitionWithId(name: String, isUnion: Boolean): FeatureExpr =
      env.get(name, isUnion).map(_.when(_.id.isDefined)).getOrElse(False)
  ...
  type EnumEnv = Map[String, (FeatureExpr, Id)]
