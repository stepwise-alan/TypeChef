...
class CTypeSystemFrontend(iast: TranslationUnit,
  ...
  def makeSilent(): CTypeSystemFrontend = {
    isSilent = true
    this
  }
  ...
  def dbgPrint(o: Any): Unit = {
    if (DEBUG_PRINT) print(o)
  }

  def dbgPrintln(o: Any): Unit = {
  ...
  override def issueTypeError(severity: Severity.Severity, condition: FeatureExpr, msg: String, where: AST, severityExtra: String = ""): Unit = {
