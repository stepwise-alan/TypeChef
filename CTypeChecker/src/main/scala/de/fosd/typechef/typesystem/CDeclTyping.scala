...
trait CDeclTyping extends CTypes with CEnv with CTypeSystemInterface with CDeclUseInterface {
  ...
  def getFunctionType(
    ...
    if (isTypedef(specifiers)) One(CUnknown("Invalid typedef specifier for function definition (?)"))
    ...
    for (Opt(f, oldParam) <- oldStyleParameters)
      oldParam match {
        case declaration: Declaration =>
          for ((name, fexpr, ast, ptype, _, _) <- getDeclaredVariables(declaration, featureExpr and f, env)._2)
  ...
  private def hasTransparentUnionAttribute(specifiers: List[Specifier]): Boolean =
    specifiers.exists(isTransparentUnionAttribute)

  private def hasTransparentUnionAttributeOpt(specifiers: List[Opt[Specifier]]): Boolean =
    specifiers.exists(o => isTransparentUnionAttribute(o.entry))

  private def isTransparentUnionAttribute(specifier: Specifier): Boolean =
    specifier match {
  ...
  private def containsAtomicAttribute(f: String => Boolean, attrs: List[Opt[AttributeSequence]]): Boolean =
    (for (Opt(_, attrSeq) <- attrs; Opt(_, attr) <- attrSeq.attributes)
      yield attr match {
  ...
  private def constructTypeOne(specifiers: List[Specifier], featureExpr: FeatureExpr,
    ...
    if (types.size == 1)
      ...
      //            all occurrences or add this rule to the typesystem.
  ...
  private def noInitCheck = (_: Expr, _: Conditional[CType], _: FeatureExpr, _: Env) => {}
  ...
  protected def checkStructCompleteness(ctype: CType, expr: FeatureExpr, env: Env, where: AST, checkedStructs: List[String] = Nil): Unit = ctype.atype match {
    ...
    case CAnonymousStruct(_, fields, _) => //check fields
      ...
      fieldTypes.foreach(ct => checkStructCompletenessC(ct, expr, env, where, checkedStructs))
    ...
    val varDecl: scala.List[(String, FeatureExpr, AST, Conditional[CType], DeclarationKind, Conditional[Linkage])] =
      if (isTypedef(decl.declSpecs)) List() //no declaration for a typedef
        ...
        for (Opt(f, init) <- decl.init) yield {
          ...
          val linkage = getLinkage(init.getName, isFunctionDef = false, decl.declSpecs, featureExpr and f, env, decl)

          eenv = eenv.addVar(init.getName, featureExpr and f, init, ctype,
            declKind, env.scope, linkage)

          init.getExpr foreach {
   ...
   * */
  protected def enumDeclarations(specs: List[Opt[Specifier]], featureExpr: FeatureExpr, @unused d: AST, env: Env): List[(String, FeatureExpr, AST, Conditional[CType], DeclarationKind, Conditional[Linkage])] = {
    var localEnv = env
    ...
    for (Opt(f, spec) <- specs) spec match {
      case EnumSpecifier(_, Some(enums)) =>
        for (Opt(f2, enum) <- enums) {
          enum.assignment.foreach(checkEnumInitializer(_, f and f2 and featureExpr, localEnv))
  ...
  protected def checkEnumInitializer(initializer: Expr, fexpr: FeatureExpr, env: Env): Unit
  ...
  def isTypedef(specs: List[Opt[Specifier]]): Boolean = specs.map(_.entry).contains(TypedefSpecifier())


  //shorthand
  protected def getDeclarationType(specifiers: List[Opt[Specifier]], decl: Declarator, featureExpr: FeatureExpr, env: Env): Conditional[CType] =
  ...
  protected def getDeclaratorType(decl: Declarator, returnType: Conditional[CType], featureExpr: FeatureExpr, env: Env, oldStyleParameterTypes: ConditionalTypeMap = null): Conditional[CType] = {
    ...
    decl match {
      case AtomicNamedDeclarator(_, name, _) =>
        addDefinition(name, env, featureExpr)
        rtype
      case NestedNamedDeclarator(_, innerDecl, _, _) => getDeclaratorType(innerDecl, rtype, featureExpr, env)
    }
  }
  ...
  private def getAbstractDeclaratorType(decl: AbstractDeclarator, returnType: Conditional[CType], featureExpr: FeatureExpr, env: Env): Conditional[CType] = {
    ...
    //cf. http://www.ericgiguere.com/articles/reading-c-declarations.html
    decl match {
      case AtomicAbstractDeclarator(_, _) => rtype
      case NestedAbstractDeclarator(_, innerDecl, _, _) => getAbstractDeclaratorType(innerDecl, rtype, featureExpr, env)
    }
  }
    ...
    vfoldRight(extensions.reverse, t, featureExpr,
      (fexpr: FeatureExpr, ext: DeclaratorExtension, rtype: CType) => ext match {
        case o@DeclIdentifierList(idList) =>
          if (idList.nonEmpty && oldStyleParameterTypes == null)
  ...
  private def decorateDeclaratorPointer(t: Conditional[CType], pointers: List[Opt[Pointer]]): Conditional[CType] =
    ConditionalLib.vfoldRightS(pointers, t, (_: Pointer, b: CType) => b.map(CPointer))
    ...
    for (Opt(f, structDeclaration) <- members) {
      for (Opt(g, structDeclarator) <- structDeclaration.declaratorList)
        structDeclarator match {
          ...
          case StructInitializer(_, _) => //TODO check: ignored for now, does not have a name, seems not addressable. occurs for example in struct timex in async.i test
        }
      ...
      if (structDeclaration.declaratorList.isEmpty) {
        def inlineAnonymousStructs(t: Conditional[CType]): Unit = {
          t match {
            case Choice(_, x, y) => inlineAnonymousStructs(x); inlineAnonymousStructs(y)
  ...
  //     */
  //    /**typedef environment, outside the current declaration*/
  ...
  protected def getLinkage(symbol: String, isFunctionDef: Boolean, specifiers: List[Opt[Specifier]], fexpr: FeatureExpr, env: Env, where: AST): Conditional[Linkage] = {
    ...
    issueTypeError(Severity.OtherError, fexpr and isStatic and isExtern, "static and extern specifiers cannot occur together", where)
  ...
  protected def checkVoidParameter(ctype: Conditional[CType], ctx: FeatureExpr, decl: AST): Conditional[Unit] = ctype.vmap(ctx, (ctx, ctype) => ctype.atype match {
    case CFunction(param, _) =>
      if (param.exists(isVoid))
