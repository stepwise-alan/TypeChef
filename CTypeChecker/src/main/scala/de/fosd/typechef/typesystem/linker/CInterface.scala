...
case class CInterface(
  ...
  lazy val importsByName: Map[String, Seq[CSignature]] = imports.groupBy(_.name)
  lazy val exportsByName: Map[String, Seq[CSignature]] = exports.groupBy(_.name)

  def sortedImports: Seq[CSignature] = imports.sortWith((a, b) => a.name < b.name || (a.name == b.name && a.pos.toString < b.pos.toString))

  def sortedExports: Seq[CSignature] = imports.sortWith((a, b) => a.name < b.name || (a.name == b.name && a.pos.toString < b.pos.toString))


  lazy val getInterfaceFeatures: Set[String] = {
    var result: Set[String] = Set()

    def addFeatures(featureExpr: FeatureExpr): Unit = {
      result = result ++ featureExpr.collectDistinctFeatures
    }

    addFeatures(featureModel)
    imports.foreach(s => addFeatures(s.fexpr))
    exports.foreach(s => addFeatures(s.fexpr))
  ...
  def pack(strictness: Strictness = LINK_STRICT): CInterface = if (isPacked) this
    ...
    CInterface(featureModel, importedFeatures -- declaredFeatures, declaredFeatures,
      packImports(eliminateInfeasible = true, strictness), packExports).setPacked()

  def packWithOutElimination(strictness: Strictness = LINK_STRICT): CInterface = if (isPacked) this
  else
    CInterface(featureModel, importedFeatures -- declaredFeatures, declaredFeatures,
      packImports(eliminateInfeasible = false, strictness), packExports).setPacked()
  ...
  protected def setPacked(): CInterface = {
    isPacked = true
    this
  }
  ...
  protected def setDedupImport(): CInterface = {
    isDedupImport = true
    this
  }
  ...
  private def _relaxType(ctype: CType): CType = ctype.atype match {
    ...
    case _: CSignSpecifier => CSigned(CInt())
    //consider all pointers and functions and arrays as equivalent
    case _: CPointer => CPointer(CVoid())
    case _: CFunction => CPointer(CVoid())
    case _: CArray => CPointer(CVoid())
    //consider all structs (not pointers to structs) as equivalent
    case _: CStruct => CStruct("")
    case _: CAnonymousStruct => CStruct("")
    //void and ... remain unmodified
    case v: CVoid => v
    case v: CVarArgs => v
    case v: CFloat => v
    case v: CDouble => v
    case _: CLongDouble => CDouble()
    case v: CIgnore => v
    case _: CUnknown => CIgnore()
    case _: CZero => CPointer(CVoid())
    case v: CBuiltinVaList => v
    case _: CCompound => CPointer(CVoid())
    case _: CBool => CSigned(CInt())
  }
  ...
  def mapFM(f: FeatureExpr => FeatureExpr): CInterface = CInterface(f(featureModel), importedFeatures, declaredFeatures, imports, exports)
  ...
  def conditional(condition: FeatureExpr): CInterface =
    this.and(condition).mapFM(condition implies)
