...
trait CTypeEnv extends CTypes with CTypeSystemInterface with CEnv with CDeclTyping {
  ...
  private def parameterTypesAtomic(decl: AtomicNamedDeclarator, featureExpr: FeatureExpr, env: Env, oldStyleParam: ConditionalTypeMap): List[(String, FeatureExpr, AST, Conditional[CType])] = {
    ...
    for (Opt(extensionFeature, extension) <- decl.extensions) extension match {
      ...
      case DeclParameterDeclList(paramDecls) =>
        for (Opt(paramFeature, param) <- paramDecls) {
          ...
          param match {
            ...
            case ParameterDeclarationAD(_, _, _) =>
              assertTypeSystemConstraint(condition = false, featureExpr and extensionFeature and paramFeature, "no name, old parameter style?", param) //TODO
      ...
      case e => assertTypeSystemConstraint(condition = false, featureExpr and extensionFeature, "other extensions not supported yet: " + e, e)
    }
    result
  ...
  def checkStructRedeclaration(@unused name: String, @unused isUnion: Boolean, @unused featureExpr: FeatureExpr, @unused scope: Int, @unused env: Env, @unused where: AST): Unit = {
  ...
  def addStructDeclarationToEnv(specifier: Specifier, featureExpr: FeatureExpr, initEnv: Env, declareIncompleteTypes: Boolean): Env = specifier match {
    ...
    case StructOrUnionSpecifier(_, None, Some(attributes), _, _) =>
  ...
  def wellformed(structEnv: StructEnv, ptrEnv: PtrEnv, atype: AType): Boolean = {
    ...
    def lastParam(p: Option[AType]) = p.isEmpty || p.contains(CVarArgs()) || wf(p.get)
    /*if (ctype.isObject) false
      ...
      case CArray(t, n) =>
        wf(t) && (t != CVoid()) && n > 0
      case CFunction(param, ret) => wf(ret.atype) && !arrayType(ret.atype) && param.forall(p => !arrayType(p) && p != CVoid().toCType) &&
  ...
  def addEnumDeclarationToEnv(specifiers: List[Opt[Specifier]], featureExpr: FeatureExpr, env: Env, isHeadless: Boolean): EnumEnv = {
    specifiers.foldRight(env.enumEnv)({
      (opt, b) => {
        ...
        typeSpec match {
          case EnumSpecifier(Some(i@Id(name)), l) if isHeadless || l.isDefined =>
            ...
            b.getOrElse(name, FeatureExprFactory.False) match {
              ...
              case Tuple2(feat: FeatureExpr, _) => ft = feat
            }
