...
trait CExprTyping extends CTypes with CEnv with CDeclTyping with CTypeSystemInterface with CDeclUseInterface {
  ...
  def getExprTypeRec(expr: Expr, featureExpr: FeatureExpr, env: Env, recurse: Boolean = false): Conditional[CType] = {
    val et = getExprTypeRec(_: Expr, featureExpr, env, recurse = true)

    def etF(e: Expr, f: FeatureExpr, newEnv: Env = env) = getExprTypeRec(e, f, newEnv, recurse = true)
    ...
    val resultType: Conditional[CType] =
      if (!featureExpr.isSatisfiable) {
        ...
        expr match {
          ...
          case c@Constant(v) =>
            if (opts.warning_long_designator && v.lastOption.contains('l'))
          ...
          case pc@PointerCreationExpr(expr) =>
            et(expr).vmap(featureExpr, {
              case (_, t) if t.isObject => t.map(CPointer)
              case (_, t) if t.isFunction => t.map(CPointer)
              case (f, e) =>
                reportTypeError(f, "invalid & on " + expr + " (" + e + ")", pc)
            })
          //*a: pointer dereferencing
          case pd@PointerDerefExpr(expr) =>
            et(expr).vmap(featureExpr, (f, x) => x.map {
              case CPointer(s@CStruct(name, isUnion)) =>
                ...
                s
              case CPointer(t) if !t.isInstanceOf[CVoid] => t
              case CArray(t, _) => t
              case i@CIgnore() => i
          ...
          case p@PostfixExpr(expr, PointerPostfixSuffix("->", i@Id(_))) =>
            ...
            ConditionalLib.vmapCombinationOp(functionType, providedParameterTypesExploded, featureExpr,
              (fexpr: FeatureExpr, funType: CType, paramTypes: List[CType]) =>
                funType.atype match {
                  ...
                  case u: CUnknown => u
                  case e =>
                    reportTypeError(fexpr, expr.toString + " is not a function, but has type " + e, pe)
                }
            )
          ...
          case ae@AssignExpr(lexpr, op, rexpr) =>
            ConditionalLib.vmapCombinationOp(et(rexpr), et(lexpr), featureExpr,
              (fexpr: FeatureExpr, rtype: CType, ltype: CType) => {
                ...
                ltype match {
                  case CType(_, true, _, _) if coerce(ltype, opType).isDefined =>
                  ...
                  case CType(_: CUnknown, _, _, _) => ltype.toValue
                  case CObj(CIgnore()) => ltype.toValue
          ...
          case pe@PostfixExpr(expr, SimplePostfixSuffix(_)) =>
            ...
            et(expr) map {
              ...
              case (_, CObj(t)) if isScalar(t) => t //apparently ++ also works on arrays
              case (_, CObj(CIgnore())) => CIgnore()
          ...
          case p@PostfixExpr(expr, ArrayAccess(idx)) =>
            ...
            et(newExpr)
          //"a"
          case StringLit(_) => One(CType(CPointer(CSignUnspecified(CChar())), isObject = true, isVolatile = false, isConstant = false))
          //++a, --a
          ...
          case LcurlyInitializer(_) => One(CCompound().toCType.toObj) //TODO more specific checks, currently just use CCompound which can be cast into any structure or array
          case _: GnuAsmExpr =>
          ...
          case BuiltinOffsetof(typename, offsetDesignators) =>
            ...
            One(CSigned(CInt()))
          case _: BuiltinTypesCompatible => One(CSigned(CInt())) //http://www.delorie.com/gnu/docs/gcc/gcc_81.html
          case b@BuiltinVaArgs(expr, typename) =>
            //check expr is of type va_list
            et(expr) vflatMap(featureExpr, {
              (fexpr, exprType) =>
                if (exprType.atype != CBuiltinVaList())
  ...
  private[typesystem] def operationType(op: String, type1: CType, type2: CType, where: AST, featureExpr: FeatureExpr, env: Env): CType = {
    ...
    (op, normalize(type1), normalize(type2)) match {
      ...
      case (o, t1, t2) if pointerArthOp(o) && isArithmetic(t1) && isArithmetic(t2) && coerce(t1, t2).contains("") => converse(t1, t2) //spec
      ...
      case (op, t1, t2) if compOp(op) && isPointer(t1) && isPointer(t2) && coerce(t1, t2).contains("") => CSigned(CInt()) //spec


      case ("*", t1, t2) if isArithmetic(t1) && isArithmetic(t2) && coerce(t1, t2).contains("") => converse(t1, t2)
      case ("/", t1, t2) if isArithmetic(t1) && isArithmetic(t2) && coerce(t1, t2).contains("") => converse(t1, t2)
      case ("%", t1, t2) if isIntegral(t1) && isIntegral(t2) && coerce(t1, t2).contains("") => converse(t1, t2)
      case ("=", _, _) if type1.isObject => type2.toValue //TODO spec says return t1?
      case (o, t1, t2) if logicalOp(o) && isScalar(t1) && isScalar(t2) => CSigned(CInt()) //spec
      case (o, t1, t2) if assignOp(o) && type1.isObject && coerce(t1, t2).contains("") => type2.toValue //TODO spec says return t1?
      ...
      case (_, t1, t2) =>
        if (t1.isUnknown || t2.isUnknown)
          CUnknown(t1.toString + " " + op + " " + t2.toString)
        else
   ...
   */
  def wider(t1: CType, t2: CType): CType =
    if (t1 < t2) t2 else t1
  ...
  private def typeFunctionCall(expr: AST, parameterTypes: Seq[CType], retType: CType, _foundTypes: List[CType],
                               funCall: PostfixExpr, featureExpr: FeatureExpr): CType = {
    ...
    //variadic macros
    if (expectedTypes.lastOption.contains(CVarArgs().toCType)) {
      expectedTypes = expectedTypes.dropRight(1)
      foundTypes = foundTypes.take(expectedTypes.size)
    }
    else if (expectedTypes.lastOption.contains(CVoid().toCType))
      expectedTypes = expectedTypes.dropRight(1)
    ...
    if (expectedTypes.size != foundTypes.size)
    ...
    else if (areParameterCompatible(foundTypes, expectedTypes)) {
      ...
      val problems = findIncompatibleParameter(foundTypes, expectedTypes).map(!_)
  ...
  private def areParameterCompatible(foundTypes: Seq[CType], expectedTypes: Seq[CType]): Boolean =
    findIncompatibleParameter(foundTypes, expectedTypes) forall (x => x)


  private def findIncompatibleParameter(foundTypes: Seq[CType], expectedTypes: Seq[CType]): Seq[Boolean] =
    (foundTypes zip expectedTypes) map {
      case (ft, et) => coerce(et, ft).contains("") || ft.isUnknown
    }
  ...
  def sizeofType(env: Env, x: AST, featureExpr: FeatureExpr): Conditional[CType] = {
    x match {
      case p@PostfixExpr(_, PointerPostfixSuffix(_, Id(_))) =>
        addStructUsageFromSizeOfExprU(p, featureExpr, env)
      case PostfixExpr(i: Id, _) =>
        addUse(i, featureExpr, env)
      case PointerDerefExpr(i: Id) =>
        // TODO: isUnion is set to true
        addUse(i, featureExpr, env)
      case PointerDerefExpr(NAryExpr(p, _)) => addStructUsageFromSizeOfExprU(p, featureExpr, env)
      case PointerDerefExpr(c: CastExpr) =>
        getExprType(c, featureExpr, env)
      case PostfixExpr(p: PostfixExpr, _) => addStructUsageFromSizeOfExprU(p, featureExpr, env)
      case tn@TypeName(_: List[Opt[Specifier]], _) =>
  ...
  private def addStructUsageFromSizeOfExprU(a: AST, featureExpr: FeatureExpr, env: Env) = {
    val et = getExprTypeRec(_: Expr, featureExpr, env, recurse = true)
    a match {
      case PostfixExpr(expr, PointerPostfixSuffix(_, i@Id(_))) =>
        et(expr).map(_.atype).vflatMap(featureExpr, {
          case (_, CAnonymousStruct(_, fields, _)) =>
            addAnonStructUse(i, fields)
            null
          case (_, CStruct(s, isUnion)) =>
            addStructUse(i, featureExpr, env, s, isUnion)
            null
          case (_, CPointer(CStruct(name, isUnion))) =>
            addStructUse(i, featureExpr, env, name, isUnion)
            null
          case (_, _) =>
            null
        })
      case PointerDerefExpr(_) =>
        print("")
      case _ =>
    }
  }
   ...
   */
  def addTypenameUse(typename: TypeName, offsetDesignators: List[Opt[OffsetofMemberDesignator]], env: Env): Unit =
