...
case class CType(
  ...
  def toValueLinker: CType = if (!isObject) this else copy(isObject = false)
  ...
  def map(f: AType => AType): CType = copy(atype = f(this.atype))
  ...
  override def equals(that: Any): Boolean = that match {
    ...
    case _: AType => throw new RuntimeException("comparison between CType and AType")
   ...
   */
  def equalsAType(thattype: CType): Boolean =
   ...
   */
  def equalsCType(that: CType): Boolean =
   ...
   */
  def equalsWithConst(that: CType): Boolean =
    this.atype == that.atype &&
      this.isConstant == that.isConstant
   ...
   */
  def equalsWithConstAndVolatile(that: CType): Boolean =
    this.atype == that.atype &&
      this.isConstant == that.isConstant &&
      this.isVolatile == that.isVolatile


  override def hashCode(): Int = atype.hashCode()
...
case class CChar() extends CBasicType {
  def <(that: CBasicType): Boolean = that == CLongLong() || that == CInt() || that == CLong() || that == CInt128()
...
case class CShort() extends CBasicType {
  ...
  def toXML: Elem = <short/>
}

case class CInt() extends CBasicType {
  def <(that: CBasicType): Boolean = that == CLongLong() || that == CLong() || that == CInt128()
...
case class CLong() extends CBasicType {
  def <(that: CBasicType): Boolean = that == CLongLong() || that == CInt128()
...
case class CLongLong() extends CBasicType {
  def <(that: CBasicType): Boolean = that == CInt128()

  override def toText = "long long"

  def toXML: Elem = <longlong/>
}
...
case class CSigned(b: CBasicType) extends CSignSpecifier(b) {
  override def <(that: AType): Boolean = that match {
    case CSigned(thatb) => b < thatb
    case _ => false
  }

  override def toText: String = "signed " + b.toText

  def toXML: Elem = <signed>
    {b.toXML}
  </signed>
}
...
case class CUnsigned(b: CBasicType) extends CSignSpecifier(b) {
  ...
  override def toText: String = "unsigned " + b.toText

  def toXML: Elem = <unsigned>
    {b.toXML}
  </unsigned>
}
...
case class CSignUnspecified(b: CBasicType) extends CSignSpecifier(b) {
  ...
  override def toText: String = b.toText

  def toXML: Elem = <nosign>
    {b.toXML}
  </nosign>
}
...
case class CArray(t: AType, length: Int = -1) extends AType {
  ...
  def toXML: Elem = <array length={length.toString}>
    {t.toXML}
  </array>
}
...
case class CStruct(s: String, isUnion: Boolean = false) extends AType {
  override def toText: String = (if (isUnion) "union " else "struct ") + s

  def toXML: Elem = <struct isUnion={isUnion.toString}>
    {s}
  </struct>
}
...
case class CAnonymousStruct(uniqueId: Int, fields: ConditionalTypeMap, isUnion: Boolean = false) extends AType {
  override def toText: String = (if (isUnion) "union " else "struct ") + "{" + fields + "}"

  def toXML: Elem = <astruct isUnion={isUnion.toString}>
    {}
  </astruct>
}
...
object CFunction {
  ...
  def unapply(value: CFunction): Option[(Seq[CType], CType)] = value match {
...
class CFunction(val param: Seq[CType], val ret: CType) extends AType {
  ...
  override def toText: String = param.map(_.toText).mkString("(", ", ", ")") + " => " + ret.toText

  def toXML: Elem = <function>
    {param.map(x => <param>
      {x.toXML}
    </param>)}<ret>
      {ret.toXML}
  ...
  def markSecurityRelevant(): CFunction = {
    securityRelevant = true
    this
  }

  override def hashCode(): Int = param.hashCode() * ret.hashCode()
  ...
  override def toString: String = "CFunction(" + param + "," + ret + ")"
}


//varargs should only occur in parameter lists
case class CVarArgs() extends AType {
  override def toText = "..."

  def toXML: Elem = <vargs/>
}
...
case class CUnknown(msg: String = "") extends AType {
  ...
  def toXML: Elem = <unknown msg={msg}/>
}
...
object CType {
  ...
  def fromXML(anode: scala.xml.NodeSeq): CType = {
    ...
    (node \ "volatile").foreach(x => {
      isVolatile = true
      node = x
    })
    (node \ "const").foreach(x => {
      isConst = true
      node = x
    })
  ...
  def fromXMLAType(node: scala.xml.NodeSeq): AType = {
    ...
    (node \ "signed").foreach(x => result = CSigned(fromXMLBasicType(x)))
    (node \ "unsigned").foreach(x => result = CUnsigned(fromXMLBasicType(x)))
    (node \ "nosign").foreach(x => result = CSignUnspecified(fromXMLBasicType(x)))
    (node \ "zero").foreach(_ => result = CZero())
    (node \ "void").foreach(_ => result = CVoid())
    (node \ "_Bool").foreach(_ => result = CBool())
    (node \ "float").foreach(_ => result = CFloat())
    (node \ "double").foreach(_ => result = CDouble())
    (node \ "longdouble").foreach(_ => result = CLongDouble())
    (node \ "vargs").foreach(_ => result = CVarArgs())
    (node \ "pointer").foreach(x => result = CPointer(fromXMLAType(x)))
    (node \ "array").foreach(x => result = CArray(fromXMLAType(x), x.attribute("length").get.head.text.toInt))
    (node \ "struct").foreach(x => result = CStruct(x.text.trim, x.attribute("isUnion").get.head.text.toBoolean))
    (node \ "astruct").foreach(x => result = CAnonymousStruct(AnonymousStructUniqueIdGen.gen, new ConditionalTypeMap(), x.attribute("isUnion").get.head.text.toBoolean)) //TODO
    (node \ "function").foreach(x => result = CFunction(
      (x \ "param").map(fromXML(_)),
      fromXML((x \ "ret").head)
    ))
    (node \ "compound").foreach(_ => result = CCompound())
    (node \ "builtinvalist").foreach(_ => result = CBuiltinVaList())
    (node \ "unkown").foreach(x => result = CUnknown(x.attribute("msg").get.text))
    (node \ "ignore").foreach(_ => result = CIgnore())
    result
  }
  ...
  def fromXMLBasicType(node: scala.xml.Node): CBasicType = {
    ...
    (node \ "char").foreach(_ => result = CChar())
    (node \ "short").foreach(_ => result = CShort())
    (node \ "long").foreach(_ => result = CLong())
    (node \ "longlong").foreach(_ => result = CLongLong())
    (node \ "int128").foreach(_ => result = CInt128())
    result
  }
  ...
  /**
   * checks whether two (function) types are compatible and can be linked together
   */
...
class ConditionalTypeMap(m: ConditionalMap[String, (AST, Conditional[CType])])
  ...
  def ++(that: ConditionalTypeMap): ConditionalTypeMap = if (that.isEmpty) this else new ConditionalTypeMap(this.m ++ that.m)

  def ++(l: Seq[(String, FeatureExpr, (AST, Conditional[CType]))]): ConditionalTypeMap = if (l.isEmpty) this else new ConditionalTypeMap(m ++ l)
...
class ConditionalVarEnv(m: ConditionalMap[String, (AST, Conditional[(CType, DeclarationKind, Int, Linkage)])])
  ...
  def ++(v: Seq[(String, FeatureExpr, AST, Conditional[CType], DeclarationKind, Int, Linkage)]): ConditionalVarEnv =
...
trait CTypes extends COptionProvider {
  ...
  /**
   * determines whether types are compatible in assignments etc
   *
  ...
  private def funCompatible(t1: AType, t2: AType): Boolean = (t1, t2) match {
    ...
    case (CFunction(plist1, ret1), CFunction(plist2, ret2)) =>
      coerce(ret1, ret2).contains("") && (plist1.size == plist2.size) && (plist1 zip plist2).forall(x => coerce(x._1, x._2).contains(""))
    case _ => false
  }
   ...
   * if coerce(x,y) return converse(x,y) should yield the default behavior )
   *
  ...
  def promote(x: CType): CType = converse(x, x)
  ...
  private def normalizeA(t: AType): AType = t match {
    ...
    case CArray(g, _) => normalizeA(CPointer(g)) //TODO do this recursively for all occurrences of Array
