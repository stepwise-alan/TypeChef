...
import de.fosd.typechef.typesystem.{CType, CTypeSystem, KParameter}
...
trait CInferInterface extends CTypeSystem with InterfaceWriter {
  ...
  def getInferredInterface(fm: FeatureExpr = FeatureExprFactory.True, strictness: Strictness = LINK_STRICT): CInterface = {
    cleanImports()
    new CInterface(fm, featureNames, Set(), imports, exports).pack(strictness)
  }


  var exports: List[CSignature] = List[CSignature]()
  var staticFunctions: List[CSignature] = List[CSignature]()
  var imports: List[CSignature] = List[CSignature]()
  val featureNames: Set[String] = Set[String]()
  ...
  def getExtraFlags(functionDef: FunctionDef, ctx: FeatureExpr): Set[CFlag] = {
    val flags = for (Opt(_, g@GnuAttributeSpecifier(_)) <- functionDef.specifiers;
   ...
   */
  override protected def typedFunction(fun: FunctionDef, funType: Conditional[CType], featureExpr: FeatureExpr): Unit = {
   ...
   */
  override protected def typedExpr(expr: Expr, ctypes: Conditional[CType], featureExpr: FeatureExpr, env: Env): Unit = {
