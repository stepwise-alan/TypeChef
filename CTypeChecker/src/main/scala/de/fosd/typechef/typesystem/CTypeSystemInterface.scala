...
trait CTypeSystemInterface extends CEnv {
   ...
   */
  protected def typedExpr(expr: Expr, ctype: Conditional[CType], featureExpr: FeatureExpr, env: Env): Unit = {}

  /**
   * invoked after typing a function definition (top level declaration, not nested function)
   */
  protected def typedFunction(fun: FunctionDef, ctype: Conditional[CType], featureExpr: FeatureExpr): Unit = {}

  /**
   * invoked before checking an external declaration (for example used for progress reports)
   */
  protected def checkingExternal(externalDef: ExternalDef): Unit = {}
   ...
   */
  protected def addEnv(ast: AST, env: Env): Unit = {}


  /**
   * error reporting for type errors
   */
  protected def issueTypeError(severity: Severity.Severity, condition: FeatureExpr, msg: String, where: AST, severityExtra: String = ""): Unit = {}
