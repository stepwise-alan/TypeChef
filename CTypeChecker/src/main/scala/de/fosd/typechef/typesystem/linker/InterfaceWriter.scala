...
trait InterfaceWriter {
  ...
  def writeInterface(interface: CInterface, file: File): Unit = {
    val stream = new FileWriter(file)
    scala.xml.XML.write(stream, interfaceToXML(interface), "UTF-8", xmlDecl = true, null)
    stream.close()
  }


  def debugInterface(interface: CInterface, file: File): Unit = {
    val stream = new FileWriter(file)
