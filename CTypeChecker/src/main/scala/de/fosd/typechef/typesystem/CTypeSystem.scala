...
trait CTypeSystem extends CTypes with CEnv with CDeclTyping with CTypeEnv with CExprTyping with CBuiltIn with CDeclUseInterface {
  ...
  private def checkExternalDef(externalDef: ExternalDef, featureExpr: FeatureExpr, env: Env): Env = {
    ...
    externalDef match {
      ...
      case e: TypelessDeclaration => assertTypeSystemConstraint(condition = false, featureExpr, "will not occur " + e, e); env //ignore
      case d: Declaration =>
  ...
  private def checkFunction(f: CDef, specifiers: List[Opt[Specifier]], declarator: Declarator, oldStyleParameters: List[Opt[OldParameterDeclaration]], stmt: CompoundStatement, featureExpr: FeatureExpr, env: Env): (Conditional[CType], Env) = {
    ...
    val expectedReturnType: Conditional[CType] = funType.vmap(featureExpr, {
      case (_, CType(CFunction(_, returnType), _, _, _)) => returnType
    ...
    val newEnv = newEnvEnum.addVar(declarator.getName, featureExpr, f, funType, kind, newEnvEnum.scope, getLinkage(declarator.getName, isFunctionDef = true, specifiers, featureExpr, env, declarator))
    ...
    addDecl(declarator, featureExpr, newEnv, isDefinition = false)
  ...
  private def checkRedeclaration(name: String, ctype: Conditional[CType], fexpr: FeatureExpr, env: Env, where: AST, kind: DeclarationKind): Unit = {
  ...
  private def isValidRedeclaration(newType: CType, newKind: DeclarationKind, newScope: Int, prevType: CType, prevKind: DeclarationKind, prevScope: Int): Boolean = {
    ...
    (newType.atype, prevType.atype) match {
      ...
      case (CPointer(CFunction(newParam, newRet)), CPointer(CFunction(prevParam, prevRet))) if newKind == KDeclaration && prevKind == KDeclaration =>
        ...
        return (newRet equalsWithConst prevRet) && (newParam.length == prevParam.length || prevParam.isEmpty) && newParam.zip(prevParam).forall(x => x._1 equalsAType x._2)

      //function overriding a prototype or vice versa
      case (CPointer(CFunction(_, newRet)), CPointer(CFunction(_, prevRet))) if (newKind == KDefinition && prevKind == KDeclaration) || (newKind == KDeclaration && prevKind == KDefinition) =>
  ...
  def containsIgnore(aType: AType): Boolean = aType.isIgnore || (aType match {
    ...
    case CFunction(p, r) => containsIgnore(r) || p.exists(containsIgnore)
    case CAnonymousStruct(_, f, _) => f.allTypes.exists(_.exists(containsIgnore))
    case _ => false
  })
  ...
  private def checkInitializer(initExpr: Expr, expectedType: Conditional[CType], featureExpr: FeatureExpr, env: Env): Unit = {
    ...
    ConditionalLib.vmapCombinationOp(foundType, expectedType, featureExpr, {
      (f, ft: CType, et: CType) =>
        val c = coerce(et, ft)
        if (f.isSatisfiable && c.isEmpty && !ft.isUnknown)
          issueTypeError(Severity.OtherError, f, "incorrect initializer type. expected " + et + " found " + ft, initExpr)
        if (f.isSatisfiable && c.isDefined && c.get.nonEmpty && !ft.isUnknown)
  ...
  private def checkArrayExpr(d: Declaration, featureExpr: FeatureExpr, env: Env): Unit = {
  ...
  def getStmtType(stmt: Statement, featureExpr: FeatureExpr, env: Env): (Conditional[CType], Env) = {
    ...
    stmt match {
      ...
      case LocalLabelDeclaration(_) => nop
    }
  }
  ...
  override protected def checkEnumInitializer(initializer: Expr, fexpr: FeatureExpr, env: Env): Unit =
  ...
  private def checkTypeSpecifiers(specifiers: List[Opt[Specifier]], featureExpr: FeatureExpr, env: Env): Unit = {
    for (Opt(f, spec) <- specifiers)
      checkTypeSpecifier(spec, featureExpr and f, env)
  }

  def checkTypeStructDecl(@unused decl: StructDecl, @unused expr: FeatureExpr, @unused env: Env): Unit = {}

  def checkTypeStructDeclaration(declaration: StructDeclaration, expr: FeatureExpr, env: Env): Unit = {
  ...
  def checkTypePointers(pointers: List[Opt[Pointer]], expr: FeatureExpr, env: Env): Unit = {
    for (Opt(f, ptr) <- pointers)
      checkTypeSpecifiers(ptr.specifier, expr and f, env)
  }


  def checkTypeDeclaratorExtensions(declExts: List[Opt[DeclaratorExtension]], expr: FeatureExpr, env: Env): Unit = {
    for (Opt(f, declExt) <- declExts)
      checkTypeDeclaratorExtension(declExt, expr and f, env)
  }


  def checkTypeParam(declaration: ParameterDeclaration, expr: FeatureExpr, env: Env): Unit = {
    declaration match {
  ...
  def checkTypeDeclaratorExtension(declExt: DeclaratorExtension, expr: FeatureExpr, env: Env): Unit = {
    declExt match {
  ...
  private def checkTypeDeclaration(declaration: Declaration, featureExpr: FeatureExpr, env: Env): Unit = {
    if (declaration.init.nonEmpty) //do not check specifiers on headless declarations, usually used as forward declarations
  ...
  private def checkTypeOldStyleParam(declaration: OldParameterDeclaration, expr: FeatureExpr, env: Env): Unit = {
    declaration match {
  ...
  private def checkTypeFunction(specifiers: List[Opt[Specifier]], declarator: Declarator, oldStyleParam: List[Opt[OldParameterDeclaration]], expr: FeatureExpr, env: Env): Unit = {
    checkTypeSpecifiers(specifiers, expr, env)
  ...
  private def checkTypeSpecifier(specifier: Specifier, expr: FeatureExpr, env: Env): Unit = {
    specifier match {
      ...
      case StructOrUnionSpecifier(_, Some(_), enumerators, _, _) =>
