...
sealed abstract class MonotoneFW[T](env: ASTEnv, val fm: FeatureModel) extends Attribution with IntraCFG with CFGHelper {
  ...
  protected def createFreshDefinition(i: T): T = {
  ...
  protected def getFreshDefinitionFromUsage(i: T): Set[T] = {
  ...
  protected def getFreshDefinition(i: T): T = {
  ...
  protected def getOriginal(i: T): T = freshT2T.get(i)
  ...
  protected def l: Map[T, FeatureExpr] = Map[T, FeatureExpr]()
  ...
  def out(a: AST): List[(T, FeatureExpr)] = {
    val o = outfunction(a)
  ...
  def in(a: AST): List[(T, FeatureExpr)] = {
    val i = infunction(a)
...
abstract class MonotoneFWId(env: ASTEnv, udm: UseDeclMap, fm: FeatureModel) extends MonotoneFW[Id](env, fm) {
  ...
  protected def t2T(i: PGT): Id = Id(getFreshCtr.toString + "_" + i.name)

  protected def t2SetT(i: PGT): Set[Id] = {
...
abstract class MonotoneFWIdLab(env: ASTEnv, dum: DeclUseMap, udm: UseDeclMap, fm: FeatureModel) extends MonotoneFW[(Id, Int)](env, fm) {
  ...
  protected def t2T(i: PGT): (Id, Int) = i

  protected def t2SetT(i: PGT): Set[(Id, Int)] = Set(getFreshDefinition(i))
...
class IdentityHashMapCache[A] {
  ...
  def keySet: util.Set[Any] = cache.keySet()
}
