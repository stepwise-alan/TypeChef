...
trait IntraCFG extends ASTNavigation with ConditionalNavigation {
  ...
  private implicit def optList2ASTList(l: List[Opt[AST]]): List[AST] = l.map(_.entry)

  private implicit def opt2AST(s: Opt[AST]): AST = s.entry

  private implicit def condition2AST(c: Conditional[AST]): AST = childAST(c)

  class CFGCache {
    private val cache = new java.util.IdentityHashMap[Product, List[Opt[CFGStmt]]]()

    def update(k: Product, v: List[Opt[CFGStmt]]): Unit = {
      cache.put(k, v)
    }
  ...
  def predHelper(source: Product, ctx: FeatureExpr, oldres: CFGRes, env: ASTEnv): CFGRes = {
    ...
    source match {
      ...
      case WhileStatement(expr, s) => getExprPred(expr, ctx, oldres, env) ++ filterBreakStatements(s, ctx, env)
      case DoStatement(expr, s) => getExprPred(expr, ctx, oldres, env) ++ filterBreakStatements(s, ctx, env)
      case ForStatement(_, Some(expr2), _, s) => getExprPred(expr2, ctx, oldres, env) ++
        filterBreakStatements(s, ctx, env)
      case ForStatement(_, _, _, s) => oldres ++ filterBreakStatements(s, ctx, env)
      ...
      case FunctionDef(_, _, _, stmt) => predHelper(childAST(stmt), ctx, oldres, env) ++
  ...
  private def succHelper(source: AST, ctx: FeatureExpr, oldres: CFGRes, env: ASTEnv): CFGRes = {
    source match {
      ...
      case FunctionDef(_, _, _, stmt) => oldres ++ succHelper(stmt, ctx, oldres, env)

      // EXIT element
      case ReturnStatement(Some(c: CompoundStatementExpr)) => getExprSucc(c, ctx, oldres, env)
      ...
      case IfStatement(condition, _, _, _) => getCondExprSucc(condition, ctx, oldres, env)
      case ElifStatement(condition, _) => getCondExprSucc(condition, ctx, oldres, env)
    ...
    nested_ast_elem match {
      ...
      case _ =>
        ...
        surrounding_parent match {
          // loops
          case ForStatement(Some(expr1), expr2, _, s) if isPartOf(nested_ast_elem, expr1) =>
          ...
          case ForStatement(_, expr2, Some(expr3), s) if isPartOf(nested_ast_elem, expr3) =>
            if (expr2.isDefined) getExprSucc(expr2.get, ctx, oldres, env)
            else getCondStmtSucc(s, ctx, oldres, env)
          case ForStatement(_, expr2, expr3, s) if isPartOf(nested_ast_elem, s) =>
            if (expr3.isDefined) getExprSucc(expr3.get, ctx, oldres, env)
            else if (expr2.isDefined) getExprSucc(expr2.get, ctx, oldres, env)
            else getCondStmtSucc(s, ctx, oldres, env)
        ...
        case SwitchStatement(expr, _) =>
          ...
          if (env.previous(t) != null) lconds ++ getStmtPred(t, ctx, oldres, env)
            ...
            tparent match {
              ...
              case _: AST => lconds ++ getStmtPred(tparent, ctx, oldres, env)
            }
          }
      }
    }
      ...
      case _ =>
        ...
        surrounding_parent match {
          ...
          case t@WhileStatement(expr, _) =>
            nested_ast_elem match {
              case ast: AST if ast.eq(expr) => getStmtPred(t, ctx, oldres, env)
              case _ => getExprPred(expr, ctx, oldres, env)
            }
          ...
          case t@ElifStatement(condition, _) =>
            if (isPartOf(nested_ast_elem, condition)) {
              var res = oldres
  ...
  private def filterReturnStatements(c: CompoundStatement, ctx: FeatureExpr, oldres: CFGRes, env: ASTEnv): CFGRes = {
    def filterReturnStatementsHelper(a: Any): CFGRes = {
      a match {
        case ReturnStatement(Some(c: CompoundStatementExpr)) => getExprPred(c, ctx, oldres, env)
