...
trait CFGWriter {
  ...
  def writeNode(node: AST, fexpr: FeatureExpr, containerName: String): Unit

  def writeEdge(source: AST, target: AST, fexpr: FeatureExpr): Unit
  ...
  protected def writeNodeOnce(o: AST, fexpr: () => FeatureExpr, containerName: String): Unit = {
  ...
  def writeMethodGraph(m: List[(AST, List[Opt[AST]])], lookupFExpr: AST => FeatureExpr, containerName: String): Unit = {
...
class DotGraph(fwriter: Writer) extends IOUtilities with CFGWriter {
  ...
  def writeNode(o: AST, fexpr: FeatureExpr, containerName: String): Unit = {
    val op = esc(asText(o))
  ...
  protected def esc(i: String): String = {
    i.replace("\n", "\\l").
      replace("{", "\\{").
      replace("}", "\\}").
      replace("<", "\\<").
...
class ComposedWriter(writers: List[CFGWriter]) extends CFGWriter {
  def writeNode(node: AST, fexpr: FeatureExpr, containerName: String): Unit = {
    writers.foreach(_.writeNode(node, fexpr, containerName))
  }

  def writeEdge(source: AST, target: AST, fexpr: FeatureExpr): Unit = {
    writers.foreach(_.writeEdge(source, target, fexpr))
  }

  def writeFooter(): Unit = {
    writers.foreach(_.writeFooter())
  }

  def writeHeader(filename: String): Unit = {
    writers.foreach(_.writeHeader(filename))
  }
