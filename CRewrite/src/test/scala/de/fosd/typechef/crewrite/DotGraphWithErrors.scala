...
class DotGraphWithErrors(fwriter: Writer) extends DotGraph(fwriter) {
  ...
  def writeMethodGraphWithErrors(m: List[(AST, List[Opt[AST]])], env: ASTEnv, errorNodes: List[AST] = List(), errorConnections: List[(AST, AST)] = List()): Unit = {
    ...
    for ((o, csuccs) <- m) {
      ...
      for (Opt(f, succ) <- csuccs) {
        ...
        // apply specific formatting
        if (errorConnections.exists { s => s._1.eq(succ) && s._2.eq(o) }) {
