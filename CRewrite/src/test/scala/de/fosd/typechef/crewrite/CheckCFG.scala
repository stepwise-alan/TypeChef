...
object CheckCFG extends IntraCFG with CFGHelper {
  ...
  def compareSuccWithPred(lsuccs: List[(AST, CFG)], lpreds: List[(AST, CFG)], env: ASTEnv): List[CFGError] = {
    ...
    for (s <- lsuccsast)
      if (!lpredsast.exists(x => s.eq(x)))
        sdiff ::= s

    for (p <- lpredsast)
      if (!lsuccsast.exists(x => p.eq(x)))
        pdiff ::= p

    for (sdelem <- sdiff)
      errors = CFGErrorMis("is not present in preds!", sdelem, env.featureExpr(sdelem)) :: errors

    for (pdelem <- pdiff)
      errors = CFGErrorMis("is not present in succs!", pdelem, env.featureExpr(pdelem)) :: errors
    ...
    for ((a1, b1) <- succ_edges) {
      ...
      if (!isin) {
        errors = CFGErrorDir("is missing in preds", b1, env.featureExpr(b1), a1, env.featureExpr(a1)) :: errors
      }
    }
    ...
    for ((b1, a1) <- pred_edges) {
      ...
      if (!isin) {
        errors = CFGErrorDir("is missing in succs", a1, env.featureExpr(a1), b1, env.featureExpr(b1)) :: errors
      }
    }
