...
object CFGErrorOutput {
  def printCFGErrors(s: List[(AST, List[Opt[AST]])], p: List[(AST, List[Opt[AST]])], errors: List[CFGError], env: ASTEnv): Unit = {
    val sw = new StringWriter()
    val dot = new DotGraphWithErrors(sw)
    dot.writeHeader("CFGErrorDump")

    if (errors.nonEmpty) {
      val nodeErrorsOcc = errors.filter({
        _ match {
          case _: CFGErrorMis => true;
          case _ => false
        }
      })
      val connectionErrorsOcc = errors.filter({
        _ match {
          case _: CFGErrorDir => true;
          case _ => false
        }
      })
