...
class ASTNavigationTest extends Matchers with ASTNavigation with ConditionalNavigation with EnforceTreeHelper with TestHelper with Suite {
  ...
  val ast: TranslationUnit = getAST("void foo() {}" +
    ";" +
    "void bar(){foo();}")
  val foo: ExternalDef = ast.defs.head.entry
  val bar: ExternalDef = ast.defs.last.entry
  ...
  @Ignore
  def test_ast_navigation_nextopt_onlytrue(): Unit = {
  ...
  @Ignore
  def test_ast_navigation_nextopt_conditionals(): Unit = {
    val stmt0 = LabelStatement(Id("stmt0"), None)
    val stmt1 = LabelStatement(Id("stmt1"), None)
    val stmt2 = LabelStatement(Id("stmt2"), None)
    val optstmt0 = Opt(True, stmt0)
  ...
  @Ignore
  def test_ast_navigation_prev_and_next_with_Opt_and_Choice_tree(): Unit = {
