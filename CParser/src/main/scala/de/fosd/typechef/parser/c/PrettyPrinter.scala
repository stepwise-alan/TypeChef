...
object PrettyPrinter {
  ...
  sealed abstract class Doc {
    def ~(that: Doc): Cons = Cons(this, that)

    def ~~(that: Doc): Cons = this ~ space ~ that

    def *(that: Doc): Cons = this ~ line ~ that

    def ~>(that: Doc): Cons = this ~ nest(2, line ~ that)
  }
  ...
  val line: Line.type = Line
  val space: Text = Text(" ")
  val newLineForIfdefs = true

  def nest(n: Int, d: Doc): Nest = Nest(n, d)
  ...
  def layout(d: Doc): String = d match {
    ...
    case Nest(_, Empty) => layout(Empty)
    case Nest(n, Line) => "\n" + (" " * n)
    case Nest(_, Text(s)) => layout(Text(s))
  ...
  def layoutW(d: Doc, p: Writer): Unit = d match {
    ...
    case Cons(l, r) =>
      layoutW(l, p)
      layoutW(r, p)
    case Nest(_, Empty) => layoutW(Empty, p)
    case Nest(n, Line) => p.write("\n" + (" " * n))
    case Nest(_, Text(s)) => layoutW(Text(s), p)
