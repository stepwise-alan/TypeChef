...
class ASTEnv(private var astc: java.util.IdentityHashMap[Any, (Set[FeatureExpr], Product, Product, Product, List[Any])]) {
  ...
  def containsASTElem(elem: Any): Boolean = astc.containsKey(elem)

  def get(elem: Any): ASTContext = astc.get(elem)

  def featureSet(elem: Any): Set[FeatureExpr] = astc.get(elem)._1

  def featureExpr(elem: Any): FeatureExpr = featureSet(elem).foldLeft(FeatureExprFactory.True)(_ and _)

  def parent(elem: Any): Product = astc.get(elem)._2

  def previous(elem: Any): Product = astc.get(elem)._3

  def next(elem: Any): Product = astc.get(elem)._4

  def children(elem: Any): List[Any] = astc.get(elem)._5

  def keys(): Array[AnyRef] = astc.keySet().toArray

  def add(elem: Any, newelemc: ASTContext): ASTEnv = {
    ...
    for (e <- createPrevElemNextTuples(l)) {
      e match {
        case (prev, Some(elem), next) =>
          curenv = curenv.add(elem, (fexpset, parent, prev.orNull, next.orNull, elem.asInstanceOf[Product].productIterator.toList))
        case _ =>
      }
    }
