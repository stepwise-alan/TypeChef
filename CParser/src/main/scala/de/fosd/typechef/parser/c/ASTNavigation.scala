...
trait ASTNavigation {
  ...
  def filterASTElems[T <: AST](a: Any)(implicit m: ClassTag[T]): List[T] = {
    a match {
      ...
      case p: Product => p.productIterator.toList.flatMap(filterASTElems[T](_))
      case _ => List()
    }
  }
  ...
  def filterAllASTElems[T <: AST](a: Any)(implicit m: ClassTag[T]): List[T] = {
    a match {
      case p: Product if m.runtimeClass.isInstance(p) => List(p.asInstanceOf[T]) ++
        p.productIterator.toList.flatMap(filterAllASTElems[T](_))
      case l: List[_] => l.flatMap(filterAllASTElems[T](_))
      case p: Product => p.productIterator.toList.flatMap(filterAllASTElems[T](_))
      case _ => List()
    }
  }
