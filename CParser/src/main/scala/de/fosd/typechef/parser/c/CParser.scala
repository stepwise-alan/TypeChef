...
class CParser(featureModel: FeatureModel = null, debugOutput: Boolean = false) extends MultiFeatureParser(featureModel, debugOutput) {
  ...
  //parser
  val keywords: Set[String] = Set("__real__", "__imag__", "__alignof__", "__alignof", "__asm", "__asm__", "__attribute__", "__attribute",
  ...
  val predefinedTypedefs: Set[String] = Set("__builtin_va_list", "__builtin_type")

  def translationUnit: MultiParser[TranslationUnit] = externalList ^^ {
    TranslationUnit
  }
  ...
  def specifier(name: String): MultiParser[AbstractToken] = textToken(name)
  ...
  def typedefName: MultiParser[TypeDefTypeSpecifier] =
  ...
  def notypedefName: MultiParser[Id] =
  ...
  def enumSpecifier: MultiParser[EnumSpecifier] =
    textToken("enum") ~>
      ((ID ~~ LCURLY ~! (enumList ~ RCURLY) ^^ {
        ...
        | (ID ^^ (i => EnumSpecifier(Some(i), None))))
  ...
  def functionDef: MultiParser[FunctionDef] =
    optList(functionDeclSpecifiers) ~~
      ...
      compoundStatement ^^ {
      case sp ~ declarator ~ param ~ vparam ~ _ ~ _ ~ stmt => FunctionDef(sp, declarator, param ++ vparam.map(o), stmt)
    }
  ...
  def statement: MultiParser[Conditional[Statement]] = (SEMI ^^ {
    ...
    | (textToken("default") ~! COLON ^^ (_ => DefaultStatement()))
  ...
  def assignOperator: MultiParser[AbstractToken] = (ASSIGN
    | DIV_ASSIGN
    | PLUS_ASSIGN
    | MINUS_ASSIGN
    | STAR_ASSIGN
  ...
  def conditionalExpr: MultiParser[Expr] = logicalOrExpr ~! opt(QUESTION ~ opt(expr) ~ COLON ~ conditionalExpr) ^^ {
    case e ~ Some(_ ~ e2 ~ _ ~ e3) => ConditionalExpr(e, e2, e3);
    case e ~ None => e
  }

  def constExpr: MultiParser[Expr] = conditionalExpr
  ...
  def nAryExpr(innerExpr: MultiParser[Expr], operations: MultiParser[AbstractToken]): MultiParser[Expr] =
  ...
  //avoiding repetition, more parallel to named declarators
  ...
  def unaryOperator: MultiParser[AbstractToken] = (PLUS | MINUS | BNOT | LNOT
    | LAND //for label dereference (&&label)
    | textToken("__real__")
    | textToken("__imag__"))

  def postfixExpr: MultiParser[Expr] = primaryExpr ~ postfixSuffix ^^ {
    case p ~ s =>
      var result = p
      for (suffix <- s)
  ...
  def ID: MultiParser[Id] = token("id", isIdentifier) ^^ {
    t => Id(t.getText)
  }

  def isIdentifier(token: AbstractToken): Boolean = token.isKeywordOrIdentifier && !keywords.contains(token.getText)
  ...
  def offsetofMemberDesignatorExt: MultiParser[OffsetofMemberDesignator] =
    (DOT ~> ID ^^ {
      ...
      (LBRACKET ~> expr <~ RBRACKET ^^ {
        OffsetofMemberDesignatorExpr
      })

  //
  def ASSIGN: AtomicParser[AbstractToken] = textToken('=')

  def COLON: AtomicParser[AbstractToken] = textToken(':')

  def COMMA: AtomicParser[AbstractToken] = textToken(',')

  def QUESTION: AtomicParser[AbstractToken] = textToken('?')

  def SEMI: AtomicParser[AbstractToken] = textToken(';')

  def PTR: MultiParser[AbstractToken] = textToken("->")

  def VARARGS: MultiParser[AbstractToken] = textToken("...")

  def DOT: MultiParser[AbstractToken] = textToken(".")

  def LPAREN: AtomicParser[AbstractToken] = textToken('(')

  def RPAREN: AtomicParser[AbstractToken] = textToken(')')

  def LBRACKET: AtomicParser[AbstractToken] = textToken('[')

  def RBRACKET: AtomicParser[AbstractToken] = textToken(']')

  def LCURLY: AtomicParser[AbstractToken] = textToken('{')

  def RCURLY: AtomicParser[AbstractToken] = textToken('}')

  //
  def EQUAL: MultiParser[AbstractToken] = textToken("==")

  def NOT_EQUAL: MultiParser[AbstractToken] = textToken("!=")

  def LTE: MultiParser[AbstractToken] = textToken("<=")

  def LT: MultiParser[AbstractToken] = textToken("<")

  def GTE: MultiParser[AbstractToken] = textToken(">=")

  def GT: MultiParser[AbstractToken] = textToken(">")

  //
  def DIV: AtomicParser[AbstractToken] = textToken('/')

  def DIV_ASSIGN: MultiParser[AbstractToken] = textToken("/=")

  def PLUS: AtomicParser[AbstractToken] = textToken('+')

  def PLUS_ASSIGN: MultiParser[AbstractToken] = textToken("+=")

  def INC: MultiParser[AbstractToken] = textToken("++")

  def MINUS: AtomicParser[AbstractToken] = textToken('-')

  def MINUS_ASSIGN: MultiParser[AbstractToken] = textToken("-=")

  def DEC: MultiParser[AbstractToken] = textToken("--")

  def STAR: AtomicParser[AbstractToken] = textToken('*')

  def STAR_ASSIGN: MultiParser[AbstractToken] = textToken("*=")

  def MOD: AtomicParser[AbstractToken] = textToken('%')

  def MOD_ASSIGN: MultiParser[AbstractToken] = textToken("%=")

  def RSHIFT: MultiParser[AbstractToken] = textToken(">>")

  def RSHIFT_ASSIGN: MultiParser[AbstractToken] = textToken(">>=")

  def LSHIFT: MultiParser[AbstractToken] = textToken("<<")

  def LSHIFT_ASSIGN: MultiParser[AbstractToken] = textToken("<<=")

  //
  def LAND: MultiParser[AbstractToken] = textToken("&&")

  def LNOT: AtomicParser[AbstractToken] = textToken('!')

  def LOR: MultiParser[AbstractToken] = textToken("||")

  //
  def BAND: AtomicParser[AbstractToken] = textToken('&')

  def BAND_ASSIGN: MultiParser[AbstractToken] = textToken("&=")

  def BNOT: AtomicParser[AbstractToken] = textToken('~')

  def BOR: AtomicParser[AbstractToken] = textToken('|')

  def BOR_ASSIGN: MultiParser[AbstractToken] = textToken("|=")

  def BXOR: AtomicParser[AbstractToken] = textToken('^')

  def BXOR_ASSIGN: MultiParser[AbstractToken] = textToken("^=")

  def pragma: MultiParser[Pragma] = textToken("_Pragma") ~! LPAREN ~> stringConst <~ RPAREN ^^ {
    Pragma
  }
  ...
  def gnuAsmExprWithGoto: MultiParser[GnuAsmExpr] =
    asm ~ opt(volatile) ~ textToken("goto") ~
      ...
      RPAREN ^^ {
      case _ ~ v ~ _ ~ _ ~ e ~ stuff ~ _ => GnuAsmExpr(v.isDefined, isGoto = true, e, stuff)
    }
  ...
  def gnuAsmExprWithoutGoto: MultiParser[GnuAsmExpr] =
    asm ~ opt(volatile) ~
      ...
      RPAREN ^^ {
      case _ ~ v ~ _ ~ e ~ stuff ~ _ => GnuAsmExpr(v.isDefined, isGoto = false, e, stuff)
    }

  //GCC requires the PARENs
  def strOptExprPair: MultiParser[Option[Id] ~ StringLit ~ Option[Expr]] =
  ...
  def initializerDesignation: MultiParser[InitializerElementLabel] =
    (rep1(designator) <~~ ASSIGN ^^ {
      InitializerAssigment
    }) |
      arrayDesignator |
      (ID <~~ COLON ^^ {
        InitializerDesignatorC
      })

  def designator: MultiParser[InitializerElementLabel] =
    arrayDesignator |
      (DOT ~~> ID ^^ {
        InitializerDesignatorD
      })
  ...
  def attributeKw: MultiParser[AbstractToken] = textToken("__attribute__") |
    textToken("__attribute")
  //XXX: PG: not specified anywhere by GCC docs, but used in Linux.

  def typeof: MultiParser[AbstractToken] = textToken("typeof") | textToken("__typeof") | textToken("__typeof__")

  def volatile: MultiParser[VolatileSpecifier] = (specifier("volatile") | specifier("__volatile") | specifier("__volatile__")) ^^^ VolatileSpecifier()

  def asm: MultiParser[AbstractToken] = textToken("asm") | textToken("__asm") | textToken("__asm__")

  def const: MultiParser[ConstSpecifier] = (specifier("const") | specifier("__const") | specifier("__const__")) ^^^ ConstSpecifier()

  def restrict: MultiParser[RestrictSpecifier] = (specifier("restrict") | specifier("__restrict") | specifier("__restrict__")) ^^^ RestrictSpecifier()

  def thread: MultiParser[ThreadSpecifier] = specifier("__thread") ^^^ ThreadSpecifier()

  def signed: MultiParser[SignedSpecifier] = (textToken("signed") | textToken("__signed") | textToken("__signed__")) ^^^ SignedSpecifier()

  def inline: MultiParser[InlineSpecifier] = (specifier("inline") | specifier("__inline") | specifier("__inline__")) ^^^ InlineSpecifier()

  def alignof: MultiParser[AbstractToken] = textToken("__alignof__") | textToken("__alignof")

  def ignoreAttributes(s: Specifier): Boolean = s match {
    case _: AttributeSpecifier => false
    case _ => true
  }
  ...
  def expectType: MultiParser[EmptyExternalDef] = textToken("__expectType") ~ LBRACKET ~ COLON ~!> typedefName <~ COLON <~ RBRACKET ^^ {
    _ => EmptyExternalDef()
  }

  def expectNotType: MultiParser[EmptyExternalDef] = textToken("__expectNotType") ~ LBRACKET ~ COLON ~!> notypedefName <~ COLON <~ RBRACKET ^^ {
    _ => EmptyExternalDef()
  }
  ...
  def textToken(t: Char): AtomicParser[AbstractToken] =
