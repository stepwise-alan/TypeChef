...
trait Position extends Serializable {
  ...
  def <(that: Position): Boolean = (this.getLine < that.getLine) || ((this.getLine == that.getLine) && (this.getColumn < that.getColumn))

  override def toString: String = getFile + ":" + getLine + ":" + getColumn
  ...
  override def hashCode: Int = getLine * 127 + getColumn
}
...
trait WithPosition {
  def getFile: Option[String] = {
    if (range.isDefined) Some(range.get._1.getFile) else None
  }
  ...
  def rangeClean: (Position, Position) = if (hasPosition) range.get else (NoPosition, NoPosition)
}
