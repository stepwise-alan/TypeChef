...
// * which is again based on the publicly available JavaCC grammar
// */
//
class JavaParser extends MultiFeatureParser {
  ...
  def CompilationUnit: MultiParser[Conditional[JCompilationUnit]] =
    PackageDeclaration ~ repOpt(ImportDeclaration) ~ repOpt(TypeDeclaration) ^^! {
      case p ~ imp ~ td => JCompilationUnit(p, imp, td)
    }

  def PackageDeclaration: MultiParser[Conditional[Option[JPackageDecl]]] =
    opt("package" ~> Name <~ ";") ^^! {
      _ map JPackageDecl
    }
  ...
  def Modifiers: MultiParser[List[Opt[JModifier]]] = repOpt(Modifier)

  def Modifier: MultiParser[JModifier] =
    (("public"
      |
      "static"
      |
  ...
  def ClassOrInterfaceDeclaration: MultiParser[TokenWrapper ~ JId ~ List[Opt[Any]] ~ List[Opt[JClassOrInterfaceType]] ~ List[Opt[JClassOrInterfaceType]] ~ List[Opt[JBodyDeclaration]]] =
  ...
  def EnumDeclaration: MultiParser[JId ~ List[Opt[JClassOrInterfaceType]] ~ Any] =
    "enum" ~> IDENTIFIER ~
  ...
  def TypeBound: MultiParser[Conditional[List[Opt[JClassOrInterfaceType]]]] =
    "extends" ~> rep1Sep(ClassOrInterfaceType, "&").!()
  ...
  def FieldDeclaration: MultiParser[JType ~ List[Opt[JVariableDeclarator]]] =
  ...
  def VariableDeclaratorId: MultiParser[JId ~ List[TokenWrapper ~ TokenWrapper]] =
    IDENTIFIER ~ repPlain("[" ~ "]")
  ...
  def MethodDeclaration: MultiParser[List[Opt[Any]] ~ JType ~ (JId ~ List[Opt[Any]] ~ List[TokenWrapper ~ TokenWrapper]) ~ List[Opt[JName]] ~ Option[JBlock]] =
  ...
  def MethodDeclarator: MultiParser[JId ~ List[Opt[Any]] ~ List[TokenWrapper ~ TokenWrapper]] =
  ...
  def ConstructorDeclaration: MultiParser[List[Opt[Any]] ~ JId ~ List[Opt[Any]] ~ List[Opt[JName]] ~ TokenWrapper ~ Option[Any] ~ List[Opt[Any]] ~ TokenWrapper] =
