...
object TokenWrapper {
  def create(javaToken: Token, afeatureExpr: FeatureExpr, file: String): TokenWrapper = {
    new TokenWrapper(
      javaToken.image,
      afeatureExpr,
...
class TokenWrapper(
  ...
  override def toString: String = "\"" + image + "\"" + (if (!getFeature.isTautology) getFeature else "")
