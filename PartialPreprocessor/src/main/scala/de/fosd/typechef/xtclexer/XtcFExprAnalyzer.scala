...
class XtcFExprAnalyzer {
  ...
  def resolveFExpr(xtcCondition: String): FeatureExpr = {
    ...
    parseResult match {
      ...
      case noSuccess: p.NoSuccess => throw new RuntimeException(s"parsing error: $noSuccess.msg when parsing feature name $xtcCondition produced by xtc")
    }
