...
import de.fosd.typechef.featureexpr.sat.{CastHelper, LazyLib}
...
class MacroFilter(flagFilters: List[String => Boolean]) {
  ...
  def setListFilter(openFeaturesFile: String): MacroFilter = {
    val openFeatures = Using(io.Source.fromFile(openFeaturesFile))(_.getLines().toSet).get
    new MacroFilter(
   ...
   */
  def flagFilter(x: String): Boolean = flagFilters.forall(_ (x))
}
...
class MacroContext[T](knownMacros: Map[String, Macro[T]], var cnfCache: Map[String, (String, Susp[FeatureExpr])], featureModel: FeatureModel, macroFilter: MacroFilter) extends FeatureProvider {
  ...
  def debugPrint(writer: PrintWriter): Unit = {
  ...
  def printStatistics: String =
    "\n\n\nStatistics (macros,macros with >1 alternative expansions,>2,>3,>4,non-trivial presence conditions):\n" +
      knownMacros.size + ";" +
      knownMacros.values.count(_.numberOfExpansions > 1) + ";" +
      knownMacros.values.count(_.numberOfExpansions > 2) + ";" +
      knownMacros.values.count(_.numberOfExpansions > 3) + ";" +
      knownMacros.values.count(_.numberOfExpansions > 4) + ";" +
      knownMacros.values.count(!_.getFeature.isTautology(featureModel)) + "\n"
 ...
 * featureExpansions: a list of macro definitions and the condition under which they are visible (should be mutually exclusive by construction)
 */
private class Macro[T](name: String, feature: FeatureExpr, var featureExpansions: List[MacroExpansion[T]]) {
  ...
  def getOther(fm: FeatureModel): List[MacroExpansion[T]] = {
    //lazy filtering
  ...
  override def toString: String = "#define " + name + " if " + feature.toString + " \n\texpansions \n" + featureExpansions.mkString("\n")

  def numberOfExpansions: Int = featureExpansions.size
}
...
class MacroExpansion[T](feature: FeatureExpr, expansion: T /* Actually, MacroData from PartialPreprocessor*/) {
  ...
  override def toString: String = "\t\t" + expansion.toString + " if " + feature.toString
