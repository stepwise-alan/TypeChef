...
public class AbstractCheckTests {
     ...
     * filename.check file
     */
    protected void testFile(@SuppressWarnings("SameParameterValue") String filename) throws IOException {
        testFile(filename, false);
    }


    protected void testFile(String filename, @SuppressWarnings("SameParameterValue") boolean debug) throws IOException {
        testFile(filename, debug, false);
    }

    protected void testFile(String filename, boolean debug, @SuppressWarnings("SameParameterValue") boolean ignoreWarning)
            throws IOException {
        ...
        String error;
        try {
            //getResource() returns a URL containing escapes. toURI().getPath() is needed to unescape them.
            //Otherwise, one gets a path where, e.g., spaces are represented by %20!
            output = lex(new VALexer.StreamSource(inputStream, Objects.requireNonNull(inputURI).getFile()),
                    debug, Objects.requireNonNull(getClass().getResource("/" + folder)).toURI().getPath(), ignoreWarning);
