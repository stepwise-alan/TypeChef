...
trait DifferentialTestingFramework extends LexerHelper {
  ...
  def analyzeFile(file: File, inclDirectory: File, debug: Boolean = false, ignoreWarnings: Boolean = false): Unit = {
    ...
    if (features.size > maxFeatures)
      System.err.println("Warning: too many features (%s; 2^%d configurations)\n using random instead of exhaustive strategy beyond %d features".format(features, features.size, maxFeatures))
    ...
    for (config <- genAllConfigurations(features)) {
      ...
      compareTokenLists(configuredvtokens, tokens, config, withCPP = false)
      ...
      compareTokenLists(configuredvtokens, cpptokens, config, withCPP = true)
    }
  ...
  def getTokensFromResult(result: LexerResult): List[LexerToken] =
    result match {
      case success: LexerSuccess => success.getTokens.asScala.toList.filter(_.isLanguageToken)
      case _ =>
        List()
    }
  ...
  def hasSuccess(conditional: Conditional[LexerResult]): Boolean = conditional.exists({
    case _: LexerSuccess => true
    case _ => false
  })


  protected def status(s: String): Unit = {}
  ...
  protected def tryAgainIfEmpty(cmd: () => Conditional[LexerFrontend.LexerResult], nrTries: Int): Conditional[LexerFrontend.LexerResult] = {
    ...
    if (nrTries > 1) {
      ...
      var failed = false
      r match {
        case success: LexerSuccess => if (!success.getTokens.asScala.toList.exists(_.isLanguageToken))
          failed = true
        case _ =>
      }
      if (failed)
  ...
  protected def lexcpp(file: File,
                       ...
                       @unused ignoreWarnings: Boolean = true,
    ...
    lex(new VALexer.StreamSource(jcppinput, "nofile"), debug = false, new File("."), ignoreWarnings = false, Map[String, String](), Set[String]())
  }
  ...
  private def getFeatures(initSet: Set[SingleFeatureExpr], conditional: Conditional[LexerResult]): Set[SingleFeatureExpr] = {
    ...
    conditional.foreach {
      case success: LexerSuccess => success.getTokens.asScala.foreach(t =>
  ...
  private def getFileContent(file: File): String = Using(io.Source.fromFile(file))(_.mkString).get
