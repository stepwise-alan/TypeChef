...
object ConditionalLib {
  ...
  def vfoldRightR[A, B](list: List[Opt[A]], init: Conditional[B], op: (A, B) => Conditional[B]): Conditional[B] =
    vfoldRight(list, init, True, (_, a: A, b: B) => op(a, b))
  ...
  def equals[T](a: Conditional[T], b: Conditional[T]): Boolean =
    mapCombination(a, b, (x: T, y: T) => x == y).when(identity).isTautology
  ...
  def equalsOp[T](a: Conditional[T], b: Conditional[T]): Boolean =
    mapCombinationOp(a, b, (x: T, y: T) => x == y).forall(identity)
  ...
  def flatten[T](optList: List[Opt[Conditional[T]]]): List[Opt[T]] = {
    val result: ListBuffer[Opt[T]] = ListBuffer()
    for (Opt(f, e) <- optList) {
  ...
  def conditionalFoldRight: (List[Opt[Nothing]], Conditional[Nothing], (Nothing, Nothing) => Nothing) => Conditional[Nothing] = vfoldRightS

  @deprecated("renamed to vfoldRightR for consistency", "0.4.0")
  def conditionalFoldRightR: (List[Opt[Nothing]], Conditional[Nothing], (Nothing, Nothing) => Conditional[Nothing]) => Conditional[Nothing] = vfoldRightR

  @deprecated("renamed to vfoldRight for consistency", "0.4.0")
  def conditionalFoldRightFR: (List[Opt[Nothing]], Conditional[Nothing], FeatureExpr, (FeatureExpr, Nothing, Nothing) => Conditional[Nothing]) => Conditional[Nothing] = vfoldRight

  @deprecated("renamed to vfoldRight for consistency", "0.4.0")
  def conditionalFoldLeftFR: (List[Opt[Nothing]], Conditional[Nothing], FeatureExpr, (FeatureExpr, Nothing, Nothing) => Conditional[Nothing]) => Conditional[Nothing] = vfoldLeft

  @deprecated("renamed to vmapCombination for consistency", "0.4.0")
  def mapCombinationF: (Conditional[Nothing], Conditional[Nothing], FeatureExpr, (FeatureExpr, Nothing, Nothing) => Nothing) => Conditional[Nothing] = vmapCombinationOp
  ...
  @deprecated("use value.toList().map(_._2) instead", "0.4.0")
  def leaves[T](t: Conditional[T]): List[T] = {
    t match {
      case One(value) => List(value)
      case Choice(_, thenBranch, elseBranch) => thenBranch.toList.map(_._2) ++ elseBranch.toList.map(_._2)
    }
  }
  ...
  def insert[T](t: Conditional[T], ctx: FeatureExpr, f: FeatureExpr, e: T): Conditional[T] = {
    t match {
      case o@One(_) =>
        if (f.isTautology || (ctx equivalentTo f)) One(e)
        else if (ctx isTautology) Choice(f, One(e), o)
