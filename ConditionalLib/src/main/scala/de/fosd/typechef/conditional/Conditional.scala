...
case class Opt[+T](condition: FeatureExpr, entry: T) {
   ...
   */
  def equivalentTo(that: Any): Boolean = that match {
    case Opt(f, e) => f.equivalentTo(condition) && (entry == e)
    case _ => false
  }

  //helper function
  def and(f: FeatureExpr): Opt[T] = if (f == null) this else Opt(condition.and(f), entry)

  def andNot(f: FeatureExpr): Opt[T] = if (f == null) this else Opt(condition.and(f.not()), entry)
...
abstract class Conditional[+T] extends Product {
   ...
   */
  def simplify: Conditional[T] = _simplify(True, FeatureExprFactory.empty)

  def simplify(ctx: FeatureExpr): Conditional[T] = _simplify(ctx, FeatureExprFactory.empty)

  def simplify(fm: FeatureModel): Conditional[T] = _simplify(True, fm)

  protected[conditional] def _simplify(context: FeatureExpr, fm: FeatureModel): Conditional[T] = this
  ...
  def mapf: (FeatureExpr, (FeatureExpr, T) => Nothing) => Conditional[Nothing] = vmap

  @deprecated("mapr is misnamed and should be replaced by flatMap", "0.4.0")
  def mapr: (T => Conditional[Nothing]) => Conditional[Nothing] = flatMap

  @deprecated("mapfr is misnamed and should be replaced by vflatMap", "0.4.0")
  def mapfr: (FeatureExpr, (FeatureExpr, T) => Conditional[Nothing]) => Conditional[Nothing] = vflatMap
...
case class Choice[+T](condition: FeatureExpr, thenBranch: Conditional[T], elseBranch: Conditional[T]) extends Conditional[T] {
  ...
  override def hashCode: Int = thenBranch.hashCode + elseBranch.hashCode

  protected[conditional] override def _simplify(ctx: FeatureExpr, fm: FeatureModel): Conditional[T] = {
