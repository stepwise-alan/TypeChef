...
class ConditionalTest {
  ...
  def testMap(): Unit = {
    ...
    assertEquals(Choice(fa, One(2), One(1)), Choice(fa, One("bb"), One("a")).vflatMap(True, (_, x) => One(x.length)))
  ...
  def testFoldExplosion(): Unit = {
    ...
    assertEquals(
      vt,
      vfoldRightR(ol, vt, (_: Int, v: String) => vt map (ot => if (v == ot) "X" else "U")))
  ...
  @Test
  def testConditionalSimplify(): Unit = {
    val t1: Conditional[Option[String]] = Choice(fa, One(Some("fa")), One(None))

    val t2 = Choice(fa, t1, One(Some("nfa")))

    println(t2)
    println(t2.simplify)
  }

  @Test def testForLiveness(): Unit = {
  ...
  @Test def testFlatten(): Unit = {
    assertEquals(6, Choice(fa, One(1), Choice(fb, One(2), One(3))).flatten((_, a, b) => a + b))
  }
