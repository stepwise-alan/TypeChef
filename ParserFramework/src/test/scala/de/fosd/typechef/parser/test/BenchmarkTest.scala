...
class BenchmarkTest extends TestCase with DigitListUtilities {
  ...
  @Test def testTokenCounter1(): Unit = expectResult(List(t("1"), t("2")), 2, 0, 0, p.twosymb)

  @Test def testTokenCounter2(): Unit = expectResult(List(t("a"), t("2")), 2, 0, 0, p.twosymb)

  //requires to backtrack once
  @Test def testTokenCounter3(): Unit = expectResult(List(t("a"), t("2")), 3, 1, 0, p.ab)

  //split parsing, but consume every token only once
  @Test def testSplitParsing1(): Unit = expectResult(List(t("1", f1), t("2", f1.not())), 2, 0, 0, p.digit)

  //split parsing, and consume last token twice
  @Test def testSplitParsing2(): Unit = expectResult(List(t("("), t("1", f1), t("2", f1.not()), t(")")), 5, 0, 1, p.parenDigit)

  //split parsing, but backtracks only in one branch -- which is considered as replicate parsing
  @Test def testSplitParsing3(): Unit = expectResult(List(t("a"), t("b", f1), t("1", f1.not())), 4, 0, 1, p.ab)

  //split parsing, backtracking as in previous, and replicate parsing at closing bracket
  @Test def testSplitParsing4(): Unit = expectResult(List(t("("), t("a"), t("b", f1), t("1", f1.not()), t(")")), 7, 0, 2, p.parenAb)

  //1 * 2 + 3_ 4_
  @Test def testExpr1(): Unit = expectResult(List(t("1"), t("*"), t("2", f1), t("3", f1.not()), t("+"), t("3")), 6, 0, 0, p.expr)

  //1 * (_ 2 + 3 )_
  @Test def testExpr2(): Unit = expectResult(List(t("1"), t("*"), t("(", f1), t("2"), t("+"), t("3"), t(")", f1)), 10, 0, 3, p.expr)

  private def expectResult(l: List[MyToken], tokenCounter: Int, backtracking: Int, replicatedParsing: Int, parser: p.MultiParser[Any]): Unit = {
    val in = p.tr(l)
    println("----")
