...
class DigitListParser extends MultiFeatureParser {
  ...
  def parse(tokens: List[MyToken]): ParseResult[Conditional[AST]] = digitList(new TokenReader[MyToken, TypeContext](tokens, 0, (), EofToken), FeatureExprFactory.True).expectOneResult
