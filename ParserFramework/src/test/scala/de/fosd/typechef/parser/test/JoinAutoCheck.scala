...
object JoinAutoCheck extends Properties("MultiParseResult") {
  ...
  val joinV: (FeatureExpr, A, A) => V = (f, a, b) => V(f, a, b)
  ...
  class JoinTest extends MultiFeatureParser {
    ...
    val tokenStream = new TokenReader[MyToken, TypeContext](List.fill(3)(new MyToken("_", True)), 0, (), EofToken)
  ...
  private def collectL(xf: FeatureExpr, xr: jt.ParseResult[A]): Map[L, FeatureExpr] =
    xr match {
      case jt.Success(r, _) => collectL(xf, r.asInstanceOf[A])
      case _ => Map()
    }
  ...
  private def assertContainsL(result: jt.MultiParseResult[Conditional[A]], f: FeatureExpr, l: L) = {
    ...
    for ((feature, r) <- result.toList(True))
      r match {
        case jt.Success(r, _) => found |= find(feature, r.asInstanceOf[Conditional[A]], f, l)
        case _ =>
      }
    found
  }
