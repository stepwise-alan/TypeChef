...
abstract class MultiFeatureParser(val featureModel: FeatureModel = null, debugOutput: Boolean = false) {
  ...
  class SeqParser[T, U](thisParser: => MultiParser[T], thatParser: => MultiParser[U]) extends MultiParser[~[T, U]] {
    ...
    def a: MultiParser[T] = thisParser

    def b: MultiParser[U] = thatParser
  }
  ...
  class AltParser[T, U >: T](thisParser: => MultiParser[T], alternativeParser: => MultiParser[U]) extends MultiParser[U] {
    ...
    def b: MultiParser[U] = alternativeParser
    ...
    def apply(in: Input, feature: FeatureExpr): MultiParseResult[U] = {
      ...
      result.mapfr(FeatureExprFactory.True, (f, r) => r match {
        case Success(t, restIn) =>
          t match {
            case position: WithPosition => position.setPositionRange(in.skipHidden(f, featureSolverCache).pos, restIn.pos)
            case _ =>
          }
          null
        case _ => null
  ...
  abstract class MultiParser[+T] extends ((Input, ParserState) => MultiParseResult[T]) {
    ...
    //replace on success
    def ^^^[U](repl: U): MultiParser[U] = mapWithPosition(_ => repl)
     ...
     * @return rep(this)
     */
    def * : MultiParser[List[Opt[T]]] = repOpt(this)
     ...
     */
    def *[U >: T](sep: => MultiParser[(U, U) => U]): MultiParser[List[Opt[T]]] = repSep(this, sep)
     ...
     * @return rep1(this)
     */
    def + : MultiParser[List[Opt[T]]] = rep1(this)

    /** Returns a parser that optionally parses what this parser parses.
     *
     * @return opt(this)
     */
    def ? : OptParser[T] = opt(this)
  ...
  def repPlain[T](p: => MultiParser[T]): MultiParser[List[T]] = new MultiParser[List[T]] {
    ...
    private case class Sealable[U](isSealed: Boolean, list: List[U])

    private def seal(list: List[T]) = Sealable(isSealed = true, list)

    private def unsealed(list: List[T]) = Sealable(isSealed = false, list)
...
* would take quadratic time; therefore, add x to the head and remember to reverse the list at the end. */
   ...
   * During a split we simply replicate all previous entries, which may include entries that
   ...
   * However, when we perform crosstree joins, invalid entries suddenly can matter and should be filtered.
   * (This indicates potential inefficiencies where we actually perform replication in the AST)
   *
   *
  ...
  def repOpt[T](p: => MultiParser[T], productionName: String = ""): MultiParser[List[Opt[T]]] = new RepParser[T](p) {
    ...
    def apply(in: Input, ctx: ParserState): MultiParseResult[List[Opt[T]]] = {
      ...
      var res: MultiParseResult[Sealable] = Success(Sealable(isSealed = false, List()), in)
      ...
      while (anyUnsealed(res)) {
        ...
        res.toList(ctx).foldLeft(Integer.MAX_VALUE) {
          case (x, (_, Success(t, next))) => if (t.isSealed) x else min(x, next.offset)
          case (x, _) => x
        }
            ...
            if (lastSuccess.result.isSealed || lastSuccess.next.offset != nextTokenOffset)
              ...
              applyStrategyA(lastSuccess.nextInput, ctx) match {
                case Some((result, next)) =>
                  Success(Sealable(isSealed = false, result :: lastSuccess.result.resultList), next)
                case None =>
                  ...
                  newResult map {
                    case Sealable(_, resultList) ~ Some(t) =>
                      ...
                      Sealable(isSealed = false, Opt(True, t) :: resultList) //opt true, because it will be qualified anywhen when joining
                    case Sealable(_, resultList) ~ None => Sealable(isSealed = true, resultList)
                  }
              }
            })
        //aggressive joins
    ...
    private def debug_printResult(res: MultiParseResult[Sealable], indent: Int): String =
      (" " * indent * 2) + "- " + (res match {
        ...
        case noSuccess: NoSuccess => "F " + noSuccess.nextInput.pos + " - " + noSuccess.msg + "\n"
      })
      ...
      if (!featureSolverCache.implies(ctx, firstFeature) && !featureSolverCache.mutuallyExclusive(ctx, firstFeature)) {
        ...
        parseResult match {
          case Success(result, next) =>
            if (next.offset <= in0.skipHidden(ctx.and(firstFeature.not()), featureSolverCache).offset)
  ...
  def nonEmpty[T](p: => MultiParser[List[T]]): MultiParser[List[T]] = new OtherParser[List[T]](p) {
    def apply(in: Input, feature: FeatureExpr): MultiParseResult[List[T]] = {
      p(in, feature).seqAllSuccessful(feature,
        (_: FeatureExpr, x: Success[List[T]]) =>
          if (x.result.isEmpty)
   ...
   */
  def alwaysNonEmpty[T](p: => MultiParser[List[Opt[T]]], filterE: T => Boolean = (_: T) => true): MultiParser[List[Opt[T]]] = new OtherParser[List[Opt[T]]](p) {
  ...
  def repSepOpt[T](p: => MultiParser[T], separator: => MultiParser[Elem], productionName: String = ""): MultiParser[List[Opt[T]]] =
    repSepOptIntern(firstOptional = true, p, separator, productionName) ^^ (_._1)

  /** see repSepOptIntern, consumes tailing separator(!) * */
  def rep1SepOpt[T](p: => MultiParser[T], separator: => MultiParser[Elem], productionName: String = ""): MultiParser[List[Opt[T]]] =
    repSepOptIntern(firstOptional = false, p, separator, productionName) ^^ (_._1)
   ...
   *
   */
  def repSepOptIntern[T](firstOptional: Boolean, p: => MultiParser[T], separator: => MultiParser[Elem], productionName: String = ""): MultiParser[(List[Opt[T]], ParserState)] = new MultiParser[(List[Opt[T]], FeatureExpr)] {
    thisParser =>
    ...
    def apply(in: Input, ctx: ParserState): MultiParseResult[(List[Opt[T]], FeatureExpr)] = {
      var res: MultiParseResult[Sealable] = Success(Sealable(isSealed = false, List(), ctx), in)
            ...
            if (x.result.isSealed || x.next.offset != nextTokenOffset)
              ...
              if (nextSep.isInstanceOf[Success[_]]) {
                ...
                if (featureSolverCache.mutuallyExclusive(freeSepBefore, parsingCtx))
                  ...
                  Success(Sealable(isSealed = true, x.result.resultList, freeSepBefore), nextToken)
              } else
                x
            }
        )
          ...
          (fs, x) =>
            ...
            if (x.result.isSealed || x.next.offset != nextTokenOffset)
              ...
              applyStrategyA(x.nextInput, fs) match {
                case Some((result, next)) =>
                  Success(Sealable(isSealed = false, result :: x.result.resultList, x.result.freeSeparator andNot result.condition), next)
                case None =>
                  //default case, use normal mechanism
                  // extend unsealed lists with the next result (if there is no next result, seal the list)
                  x.seq(fs, opt(p)(x.next, fs)).mapfr(fs, (f, r) => r match {
                    case Success(Sealable(_, resultList, openSep) ~ Some(t), in) =>
                      ...
                      if (featureSolverCache.implies(f, openSep))
                        Success(Sealable(isSealed = false, Opt(f, t) :: resultList, openSep andNot f), in)
                      else
                        Success(Sealable(isSealed = true, resultList, openSep), x.nextInput)
                    case Success(Sealable(_, resultList, openSep) ~ None, in) => Success(Sealable(isSealed = true, resultList, openSep), in)
                    case e: NoSuccess => e
                  })
              }
            })
      ...
      if (!firstOptional)
        result.mapfr(ctx, (_, r) => r match {
          ...
          case Success((l, _), next) if l.isEmpty => Failure("empty list (" + productionName + ")", next, List())
   ...
   */
  def lookahead[T](p: => MultiParser[T]): MultiParser[Any] = (in: Input, parserState: ParserState) => {
    p(in, parserState).seqAllSuccessful(parserState,
      (_: FeatureExpr, _: Success[T]) => Success("lookahead", in))
  }

  def fail[T](msg: String): MultiParser[T] =
    (in: Input, _: FeatureExpr) => Failure(msg, in, List())

  def failc[T](msg: String): MultiParser[Conditional[T]] =
    (in: Input, _: FeatureExpr) => Failure(msg, in, List())

  def success[T](v: T): MultiParser[T] =
    MultiParser {
      (in: Input, _: FeatureExpr) => Success(v, in)
    }

  def MultiParser[T](f: (Input, FeatureExpr) => MultiParseResult[T]): MultiParser[T] =
    (in: Input, fs: FeatureExpr) => f(in, fs)

  def matchInput(p: (Elem, FeatureExpr, TypeContext) => Boolean, kind: String): AtomicParser[Elem] = new AtomicParser[Elem](kind) {
  ...
  def token(kind: String, p: Elem => Boolean): AtomicParser[Elem] = tokenWithContext(kind, (e, _, _) => p(e))

  def tokenWithContext(kind: String, p: (Elem, FeatureExpr, TypeContext) => Boolean): AtomicParser[Elem] = matchInput(p, kind)

  private
  def errorMsg(kind: String, inEl: Option[Elem], @unused ctx: TypeContext): String =
    (if (inEl.isEmpty) "reached EOF, " else "found \"" + inEl.get.getText + "\", ") + "but expected \"" + kind + "\""
  ...
  sealed abstract class MultiParseResult[+T] {
    ...
    def mapr[U](f: ParseResult[T] => MultiParseResult[U]): MultiParseResult[U] = mapfr(FeatureExprFactory.True, (_, r) => f(r))

    /**
     * joins as far as possible. joins all successful ones but maintains partially successful results.
     * keeping partially unsuccessful results is necessary to consider multiple branches for an alternative on ASTs
     * */
  ...
  case class SplittedParseResult[+T](feature: FeatureExpr, resultA: MultiParseResult[T], resultB: MultiParseResult[T]) extends MultiParseResult[T] {
     ...
     * there are no join opportunities left within only the left or right branch of a split. joining
    ...
    def joinCrosstree(parserContext: FeatureExpr): MultiParseResult[Conditional[T]] = {
      def performJoin(featureA: FeatureExpr, a: ParseResult[Conditional[T]], b: ParseResult[Conditional[T]]): Option[ParseResult[Conditional[T]]] = (a, b) match {
        case (Success(rA: Conditional[_], inA), Success(rB: Conditional[_], inB)) =>
    ...
    private def isSamePosition(parserContext: FeatureExpr, inA: TokenReader[Elem, TypeContext], inB: TokenReader[Elem, TypeContext]): Boolean = {
      ...
      nextA.offset == nextB.offset
    }

    private def firstOf(inA: TokenReader[Elem, TypeContext], inB: TokenReader[Elem, TypeContext]) =
      (if (inA.offset < inB.offset) inB else inA).setContext(joinContext(inA.context, inB.context))

    //don't create a choice when both branches are the same
    @unused
    private def createChoice[U](f: FeatureExpr, a: U, b: U): Conditional[U] = createChoiceC(f, One(a), One(b))

    private def createChoiceC[U](f: FeatureExpr, a: Conditional[U], b: Conditional[U]): Conditional[U] = if (a == b) a else Choice(f, a, b)
    ...
    private def prune[U >: T](tree: MultiParseResult[Conditional[U]], pruneList: List[ParseResult[Conditional[U]]]): MultiParseResult[Conditional[U]] =
      if (pruneList.isEmpty) tree
        ...
        case SplittedParseResult(f, a, b) =>
          val pa = prune[U](a, pruneList)
          val pb = prune[U](b, pruneList)
     ...
     * the disadvantage is that it misses opportunities for joins
     */
    def joinTree(parserContext: FeatureExpr): MultiParseResult[Conditional[T]] = {
          ...
          if (isSamePosition(parserContext, inA, inB)) {
            ...
            SplittedParseResult(
              ...
              Success(Choice(feature or innerFeature, rA, rB),
                if (inA.offset < inB.offset) inB else inA),
              otherParseResult)
          } else
            SplittedParseResult(feature, sA, sB)
        //both not successful
        case (nA@NoSuccess(_, inA, _), nB@NoSuccess(_, _, _)) =>
    ...
    def joinNoSuccess(): MultiParseResult[T] =
      (resultA.joinNoSuccess(), resultB.joinNoSuccess()) match {
        case (f1@Failure(msg1, next1, inner1), f2@Failure(_, _, inner2)) =>
          Failure(msg1, next1, List(f1, f2) ++ inner1 ++ inner2)
        case (f1@Error(msg1, next1, inner1), f2@Error(_, _, inner2)) =>
          Error(msg1, next1, List(f1, f2) ++ inner1 ++ inner2)
        case (a, b) => SplittedParseResult(feature, a, b)
      }

    def allFailed: Boolean = resultA.allFailed && resultB.allFailed

    def exists(p: T => Boolean): Boolean = resultA.exists(p) || resultB.exists(p)

    def toList(context: FeatureExpr): List[(ParserState, ParseResult[T])] = resultA.toList(context and feature) ++ resultB.toList(context and feature.not())

    def toErrorList: List[Error] = resultA.toErrorList ++ resultB.toErrorList
  ...
  sealed abstract class ParseResult[+T](nextInput: TokenReader[Elem, TypeContext]) extends MultiParseResult[T] {
    ...
    def next: TokenReader[Elem, TypeContext] = nextInput
    ...
    def joinNoSuccess(): MultiParseResult[T] = this

    def toList(context: FeatureExpr): List[(ParserState, ParseResult[T])] = List((context, this))
  ...
  abstract class NoSuccess(val msg: String, val nextInput: TokenReader[Elem, TypeContext], val innerErrors: List[NoSuccess]) extends ParseResult[Nothing](nextInput) {
    ...
    def changeContext(ctx: FeatureExpr, contextModification: (Nothing, FeatureExpr, TypeContext) => TypeContext): MultiParseResult[Nothing] = this
  }

  /** An extractor so NoSuccess(msg, next) can be used in matches.
   */
  object NoSuccess {
    def unapply(x: NoSuccess): Option[(String, TokenReader[Elem, TypeContext], List[NoSuccess])] = x match {
  ...
  case class Failure(override val msg: String, override val nextInput: TokenReader[Elem, TypeContext], override val innerErrors: List[NoSuccess]) extends NoSuccess(msg, nextInput, innerErrors) {
    def replaceAllFailure[U >: Nothing](context: FeatureExpr, f: FeatureExpr => MultiParseResult[U]): MultiParseResult[U] = f(context)

    def commit: MultiParseResult[Nothing] = Error(msg, nextInput, innerErrors)

    def toErrorList: List[Error] = List()
  }
  ...
  case class Error(override val msg: String, override val nextInput: TokenReader[Elem, TypeContext], override val innerErrors: List[NoSuccess]) extends NoSuccess(msg, nextInput, innerErrors) {
    def replaceAllFailure[U >: Nothing](context: FeatureExpr, f: FeatureExpr => MultiParseResult[U]): MultiParseResult[U] = this

    def commit: MultiParseResult[Nothing] = this

    def toErrorList: List[Error] = List(this)
  }

  case class Success[+T](result: T, nextInput: TokenReader[Elem, TypeContext]) extends ParseResult[T](nextInput) {
    ...
    def seq[U](context: FeatureExpr, thatResult: MultiParseResult[U]): MultiParseResult[~[T, U]] =
      thatResult.seqAllSuccessful[~[T, U]](context, (_: FeatureExpr, x: Success[U]) => Success(new ~(result, x.result), x.next))

    def replaceAllFailure[U >: T](context: FeatureExpr, f: FeatureExpr => MultiParseResult[U]): MultiParseResult[U] = this

    def allFailed = false

    def exists(predicate: T => Boolean): Boolean = predicate(result)
   ...
   */
  def phrase[T](p: MultiParser[T]): MultiParser[T] = (in: Input, fs: FeatureExpr) => {
    val result = p(in, fs)

    result.mapfr(fs, (_, result) =>
      result match {
