    ...
    assignment match {
      case Some((trueFeatures, falseFeatures)) =>
        ...
        if (preferDisabledFeatures) {
          ...
          Some(remainingInterestingFeatures.toList, disabledFeatures.toList)
        }
      case None => None
    }
  }
...
private object SatSolverCache {
  val cache: mutable.WeakHashMap[BDDFeatureModel, SatSolverImpl] = new mutable.WeakHashMap()

  def get(fm: BDDFeatureModel): SatSolverImpl = {
    val c = cache.get(fm)
  ...
  def isSatisfiable(dnf: Iterator[Seq[Int]], lookupName: Int => String): Boolean = {
    ...
    try {
      ...
      for (f <- featureModel.assumedTrue)
        uniqueFlagIds.get(f).map(assumptions.push)
      ...
      for (constr <- constraintGroup.filter(_ != null)) {
        try {
          ...
          case _: AssertionError => invalidateCache()
          case _: NullPointerException => invalidateCache()
        }
      }
