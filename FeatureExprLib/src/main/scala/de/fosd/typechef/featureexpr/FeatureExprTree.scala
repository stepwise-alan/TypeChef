...
private[featureexpr] class If[T](val expr: FeatureExpr, val thenBr: FeatureExprTree[T], val elseBr: FeatureExprTree[T]) extends FeatureExprTree[T] {
  override def toString: String = "(" + expr + "?" + thenBr + ":" + elseBr + ")"
...
private[featureexpr] object If {
  def unapply[T](x: If[T]): Some[(FeatureExpr, FeatureExprTree[T], FeatureExprTree[T])] = Some((x.expr, x.thenBr, x.elseBr))
}
...
private[featureexpr] object Value {
  def unapply[T](x: Value[T]): Some[T] = Some(x.value)
}
...
private[featureexpr] object ErrorValue {
  def unapply[T](x: ErrorValue[T]): Some[String] = Some(x.msg)
