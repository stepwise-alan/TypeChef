...
object FeatureExprHelper {
   ...
   */
  val cacheIsSatisfiable: mutable.WeakHashMap[(BDD, FeatureModel), Boolean] = mutable.WeakHashMap()
}
...
class BDDFeatureExpr(private[featureexpr] val bdd: BDD) extends FeatureExpr {
  ...
  def simplify(b: FeatureExpr): FeatureExpr = FExprBuilder.synchronized {
    new BDDFeatureExpr(this.bdd.simplify(asBDDFeatureExpr(b).bdd))
  }
  ...
  override def implies(that: FeatureExpr): FeatureExpr = FExprBuilder.imp(this, asBDDFeatureExpr(that))

  override def xor(that: FeatureExpr): FeatureExpr = FExprBuilder.xor(this, asBDDFeatureExpr(that))
  ...
  override def equiv(that: FeatureExpr): FeatureExpr = FExprBuilder.biimp(this, asBDDFeatureExpr(that))
  ...
  final override def equals(that: Any): Boolean = that match {
    case x: BDDFeatureExpr => FExprBuilder.synchronized {
      bdd.equals(x.bdd)
    }
    case _ => super.equals(that)
  }

  override def hashCode: Int = FExprBuilder.synchronized {
    bdd.hashCode
  }


  protected def calcSize: Int = FExprBuilder.synchronized {
    bdd.nodeCount
  }
  ...
  def getBddAllSat: Iterator[Array[(Byte, String)]] = FExprBuilder.synchronized {
    def clause(d: Array[Byte]): Array[(Byte, String)] = d.zipWithIndex.filter(_._1 >= 0).map(
      x => (x._1, FExprBuilder.lookupFeatureName(x._2))
    )

    bddAllSat.map(clause)
  }
  ...
  def toStringDNF: String =
    printbdd(bdd, "True", "False", " & ", " | ", FExprBuilder.lookupFeatureName)
  ...
  private def printbdd(bdd: BDD, one: String, zero: String, and: String, or: String, toName: Int => String): String = FExprBuilder.synchronized {
    if (bdd.isOne) one
      ...
      def clause(d: Array[Byte]): String = d.zipWithIndex.filter(_._1 >= 0).map(
        x => (if (x._1 == 0) "!" else "") + toName(x._2)
      ).mkString(and)

      return bddAllSat.map(clause).mkString(or)
    }
  }

  private def bddAllSat: Iterator[Array[Byte]] = FExprBuilder.synchronized {
    toScalaAllSat(bdd.allsat())
  }

  private def toScalaAllSat(allsat: java.util.List[_]): Iterator[Array[Byte]] =
    scala.jdk.javaapi.CollectionConverters.asScala(allsat.asInstanceOf[java.util.List[Array[Byte]]].iterator())
  ...
  private def toDnfClauses(allsat: Iterator[Array[Byte]]): Iterator[Seq[Int]] = {
    def clause(d: Array[Byte]): Seq[Int] = d.zipWithIndex.filter(_._1 >= 0).map(
  ...
  private def toCNFClauses(topbdd: BDD): Iterator[Seq[Int]] = FExprBuilder.synchronized {
    ...
    val bddIds: mutable.Map[BDD, Int] = mutable.Map()
    val incomingEdges: mutable.Map[BDD, Set[Int]] = mutable.Map()

    def addIncomingEdge(bdd: BDD, source: Int): Unit = {
  ...
  def collectDistinctFeatures: Set[String] =
    collectDistinctFeatureIds.map(FExprBuilder lookupFeatureName)
  ...
  def evaluate(selectedFeatures: Set[String]): Boolean = FExprBuilder.synchronized {
    FExprBuilder.evalBdd(bdd, selectedFeatures)
  }
...
class SingleBDDFeatureExpr(id: Int) extends BDDFeatureExpr(lookupFeatureBDD(id)) with SingleFeatureExpr {
  def feature: String = lookupFeatureName(id)
...
private[bdd] object FExprBuilder {
  ...
  val bddFactory: BDDFactory = try {
    BDDFactory.init("j", bddValNum, bddCacheSize)
  } catch {
  ...
  private val featureIds: mutable.Map[String, Int] = mutable.Map()
  private val featureNames: mutable.Map[Int, String] = mutable.Map()
  private val featureBDDs: mutable.Map[Int, BDD] = mutable.Map()


  def and(a: BDDFeatureExpr, b: BDDFeatureExpr): BDDFeatureExpr = this.synchronized {
    new BDDFeatureExpr(a.bdd and b.bdd)
  }

  def or(a: BDDFeatureExpr, b: BDDFeatureExpr): BDDFeatureExpr = this.synchronized {
    new BDDFeatureExpr(a.bdd or b.bdd)
  }

  def imp(a: BDDFeatureExpr, b: BDDFeatureExpr): BDDFeatureExpr = this.synchronized {
    new BDDFeatureExpr(a.bdd imp b.bdd)
  }

  def biimp(a: BDDFeatureExpr, b: BDDFeatureExpr): BDDFeatureExpr = this.synchronized {
    new BDDFeatureExpr(a.bdd biimp b.bdd)
  }

  def xor(a: BDDFeatureExpr, b: BDDFeatureExpr): BDDFeatureExpr = this.synchronized {
    new BDDFeatureExpr(a.bdd xor b.bdd)
  }

  def unique(a: BDDFeatureExpr, b: SingleBDDFeatureExpr): BDDFeatureExpr = this.synchronized {
    new BDDFeatureExpr(a.bdd.unique(b.bdd))
  }

  def not(a: BDDFeatureExpr): BDDFeatureExpr = this.synchronized {
    new BDDFeatureExpr(a.bdd.not())
  }
...
object True extends BDDFeatureExpr(FExprBuilder.TRUE) with DefaultPrint with SingleFeatureExpr {
  ...
  override def feature: String = toString
