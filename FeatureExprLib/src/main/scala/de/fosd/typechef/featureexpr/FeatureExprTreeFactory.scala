...
trait FeatureExprTreeFactory extends FeatureExprValueOps {
  ...
  def createNeg(expr: FeatureExprValue): FeatureExprTree[Long] = applyUnaryOperation(expr)(-_)

  def createBitAnd(left: FeatureExprValue, right: FeatureExprValue): FeatureExprTree[Long] = applyBinaryOperation(left, right)(_ & _)

  def createBitOr(left: FeatureExprValue, right: FeatureExprValue): FeatureExprTree[Long] = applyBinaryOperation(left, right)(_ | _)
  ...
  def createModulo(left: FeatureExprValue, right: FeatureExprValue): FeatureExprTree[Long] = applyBinaryOperation(left, right)(_ % _)

  def createEquals(left: FeatureExprValue, right: FeatureExprValue): FeatureExpr = evalRelation(left, right)(_ == _)

  def createNotEquals(left: FeatureExprValue, right: FeatureExprValue): FeatureExpr = evalRelation(left, right)(_ != _)

  def createLessThan(left: FeatureExprValue, right: FeatureExprValue): FeatureExpr = evalRelation(left, right)(_ < _)

  def createLessThanEquals(left: FeatureExprValue, right: FeatureExprValue): FeatureExpr = evalRelation(left, right)(_ <= _)

  def createGreaterThan(left: FeatureExprValue, right: FeatureExprValue): FeatureExpr = evalRelation(left, right)(_ > _)

  def createGreaterThanEquals(left: FeatureExprValue, right: FeatureExprValue): FeatureExpr = evalRelation(left, right)(_ >= _)

  def createMinus(left: FeatureExprValue, right: FeatureExprValue): FeatureExprTree[Long] = applyBinaryOperation(left, right)(_ - _)

  def createMult(left: FeatureExprValue, right: FeatureExprValue): FeatureExprTree[Long] = applyBinaryOperation(left, right)(_ * _)

  def createPlus(left: FeatureExprValue, right: FeatureExprValue): FeatureExprTree[Long] = applyBinaryOperation(left, right)(_ + _)

  def createPwr(left: FeatureExprValue, right: FeatureExprValue): FeatureExprTree[Long] = applyBinaryOperation(left, right)(_ ^ _)

  def createShiftLeft(left: FeatureExprValue, right: FeatureExprValue): FeatureExprTree[Long] = applyBinaryOperation(left, right)(_ << _)

  def createShiftRight(left: FeatureExprValue, right: FeatureExprValue): FeatureExprTree[Long] = applyBinaryOperation(left, right)(_ >> _)
  ...
  def evalRelation[T](smaller: FeatureExprTree[T], larger: FeatureExprTree[T])(relation: (T, T) => Boolean): FeatureExpr = {
    propagateError(smaller, larger) match {
      case Some(ErrorValue(msg)) => new ErrorFeature(msg, False)
      case _ =>
        (smaller, larger) match {
          case (a: Value[T], b: Value[T]) => if (relation(a.value, b.value)) True else False
          case (i1: If[T], i2: If[T]) =>
          ...
          case (i: If[T], x) => createBooleanIf(i.expr, evalRelation(i.thenBr, x)(relation), evalRelation(i.elseBr, x)(relation))
          case (x, i: If[T]) => createBooleanIf(i.expr, evalRelation(x, i.thenBr)(relation), evalRelation(x, i.elseBr)(relation))
          case _ => throw new Exception("evalRelation: unexpected " + (smaller, larger))
        }
    }
  }

  def applyBinaryOperation[T, U](left: FeatureExprTree[T], right: FeatureExprTree[T])(operation: (T, T) => U)(implicit function: U => FeatureExprTree[T]): FeatureExprTree[T] = {
