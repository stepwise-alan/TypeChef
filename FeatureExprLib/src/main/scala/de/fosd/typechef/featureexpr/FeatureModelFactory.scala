...
trait FeatureModelFactory {
  ...
  protected def loadDimacsData(file: Source, translateNames: String => String, autoAddVariables: Boolean): (Map[String, Int], List[List[Int]], Int) = {
    ...
    assert(clauses.size == numDeclaredClauses, "Inconsistent dimacs file: number of clauses %d differs from declared number of clauses %d".format(clauses.size, numDeclaredClauses))
    if (!autoAddVariables) {
