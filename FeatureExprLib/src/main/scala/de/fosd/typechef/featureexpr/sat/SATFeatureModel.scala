...
class SATFeatureModel(val variables: Map[String, Int], val clauses: IVec[IVecInt], val lastVarId: Int) extends FeatureModel {
  ...
  def assumeTrue(featurename: String): FeatureModel = this.and(SATFeatureExprFactory.createDefinedExternal(featurename))

  def assumeFalse(featurename: String): FeatureModel = this.and(SATFeatureExprFactory.createDefinedExternal(featurename).not())
  ...
  def exportFM2DNF(@unused fm: FeatureModel, fileName: String): Unit = {
    ...
    def posNegFeatureName(id: Int) = {
      if (mIdFlagg.isDefinedAt(id.abs))
        Some("(" + (if (id > 0) "" else "not ") + mIdFlagg(id.abs) + ")")
      else
        None
    }
  ...
  // $COVERAGE-OFF$
  def writeToDimacsFile(file: File): Unit = {
      ...
      for (clause: IVecInt <- clauses.toArray) {
        if (clause != null) {
          ...
          for (entry: Int <- clause.toArray) {
            clauseBuffer.append(entry.toString + " ")
          }
...
object SATFeatureModel extends FeatureModelFactory {
   ...
   */
  def empty: FeatureModel = SATNoFeatureModel

  /**
   * create a feature model from a feature expression
   */
  def create(expr: FeatureExpr): FeatureModel = SATNoFeatureModel.and(expr)

  /**
   * create a feature model by loading a CNF file
   * (proprietary format used previously by LinuxAnalysis tools)
   */
  def createFromCNFFile(file: String): SATFeatureModel = {
