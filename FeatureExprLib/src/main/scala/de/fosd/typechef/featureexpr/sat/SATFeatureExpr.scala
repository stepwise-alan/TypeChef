...
sealed abstract class SATFeatureExpr extends FeatureExpr {
    ...
    assignment match {
      case Some((trueFeatures, falseFeatures)) =>
        if (preferDisabledFeatures) {
          ...
          Some((enabledFeatures.toList, remainingInterestingFeatures.toList))
        } else {
          ...
          Some((remainingInterestingFeatures.++(this.collectDistinctFeatureObjects).toList, disabledFeatures.toList))
        }
      case None => None
    }
  }
  ...
  protected def calcHashCode: Int = super.hashCode
  ...
  private def calcIsResolved: Boolean = {
    ...
    try {
      ...
      case _: FoundUnresolvedException => false
    }
  }
   ...
   */
  def mapDefinedExpr(f: DefinedExpr => SATFeatureExpr, cache: mutable.Map[SATFeatureExpr, SATFeatureExpr]): SATFeatureExpr
   ...
   * in satisfiability tests with SAT solvers
   *
  ...
  private val cacheIsSatisfiable: mutable.Map[SATFeatureModel, Boolean] =
  ...
  private[sat] val andCache: mutable.Map[SATFeatureExpr, WeakReference[SATFeatureExpr]] =
    Collections.synchronizedMap(new java.util.WeakHashMap[SATFeatureExpr, WeakReference[SATFeatureExpr]]()).asScala
  private[sat] val orCache: mutable.Map[SATFeatureExpr, WeakReference[SATFeatureExpr]] =
    Collections.synchronizedMap(new java.util.WeakHashMap[SATFeatureExpr, WeakReference[SATFeatureExpr]]()).asScala
...
abstract class HashCachingFeatureExpr extends SATFeatureExpr {
  protected val cachedHash: Int = calcHashCode

  final override def hashCode: Int = cachedHash
}
...
private[sat] object FExprBuilder {
  ...
  private val featureCache: mutable.Map[String, WeakReference[DefinedExternal]] = mutable.Map()
  private val macroCache: mutable.Map[String, WeakReference[DefinedMacro]] = mutable.Map()
  private val resolvedCache: mutable.Map[SATFeatureExpr, SATFeatureExpr] =
    Collections.synchronizedMap(new java.util.WeakHashMap[SATFeatureExpr, SATFeatureExpr]()).asScala
  private val hashConsingCache: mutable.Map[StructuralEqualityWrapper, WeakReference[StructuralEqualityWrapper]] =
  ...
  private def cacheGetOrElseUpdate[A, B <: AnyRef](map: mutable.Map[A, WeakReference[B]], key: A, op: => B): B = {
    def update() = {
      val d = op
  ...
  private def getFromCache(cache: mutable.Map[SATFeatureExpr, WeakReference[SATFeatureExpr]], key: SATFeatureExpr): Option[SATFeatureExpr] = {
    cache.get(key) match {
  ...
  private def binOpCacheGetOrElseUpdate(a: SATFeatureExpr,
                                        b: SATFeatureExpr,
                                        getCache: SATFeatureExpr => mutable.Map[SATFeatureExpr, WeakReference[SATFeatureExpr]],
    ...
    (a, b) match {
      ...
      case other =>
        binOpCacheGetOrElseUpdate(a, b, _.andCache,
          other match {
            case (a1: And, a2: And) =>
              a2.clauses.foldLeft[SATFeatureExpr](a1)(fastAnd) //XXX: O(N) set rebuild
  ...
  def createAnd(clauses: Iterable[SATFeatureExpr]): SATFeatureExpr =
    canonical(clauses.foldLeft[SATFeatureExpr](True)(fastAnd))
    ...
    (a, b) match {
      ...
      case other =>
        binOpCacheGetOrElseUpdate(a, b, _.orCache,
          other match {
            case (o1: Or, o2: Or) =>
              o2.clauses.foldLeft[SATFeatureExpr](o1)(fastOr) //XXX: O(N) set rebuild
  ...
  def createOr(clauses: Iterable[SATFeatureExpr]): SATFeatureExpr =
    canonical(clauses.foldLeft[SATFeatureExpr](False)(fastOr))
  ...
  def definedExternal(name: String): DefinedExternal = cacheGetOrElseUpdate(featureCache, name, new DefinedExternal(name))
...
object True extends And(Set()) with DefaultPrint {
  ...
  override def debug_print(ind: Int): String = indent(ind) + toTextExpr + "\n"
...
abstract class AndOrUnExtractor[This <: BinaryLogicConnective[This]] {
  ...
  def unapply(x: This): Some[Set[SATFeatureExpr]] = Some(x.clauses)
...
object And extends AndOrUnExtractor[And] {
  def identity: SATFeatureExpr = True
...
object Or extends AndOrUnExtractor[Or] {
  def identity: SATFeatureExpr = False
...
abstract class BinaryLogicConnective[This <: BinaryLogicConnective[This]] extends SATFeatureExpr {
  ...
  def create(clauses: Iterable[SATFeatureExpr]): SATFeatureExpr
  //Can't declare This as return type - the optimizations in FExprBuilder are such that it might build an object of
  //unexpected type.

  override def equal1Level(that: SATFeatureExpr): Boolean = that match {
  ...
  override def calcHashCode: Int = primeHashMult * clauses.map(_.hashCode).sum
  ...
  protected def presetHash(old: This, newF: SATFeatureExpr): Unit =
  ...
  override def toString: String = clauses.mkString("(", operName, ")")

  override def toTextExpr: String = clauses.map(_.toTextExpr).mkString("(", " " + operName + operName + " ", ")")

  override def print(p: Writer): Unit = {
    p write "("
    if (clauses.nonEmpty) {
  ...
  override def debug_print(ind: Int): String = indent(ind) + operName + "\n" + clauses.map(_.debug_print(ind + 1)).mkString

  override def calcSize: Int = clauses.foldLeft(0)(_ + _.size)

  override def mapDefinedExpr(f: DefinedExpr => SATFeatureExpr, cache: mutable.Map[SATFeatureExpr, SATFeatureExpr]): SATFeatureExpr = cache.getOrElseUpdate(this, {
    var anyChange = false
...
class And(val clauses: Set[SATFeatureExpr]) extends BinaryLogicConnective[And] {
  ...
  override def create(clauses: Iterable[SATFeatureExpr]): SATFeatureExpr = FExprBuilder.createAnd(clauses)
  ...
  override def evaluate(selectedFeatures: Set[String]): Boolean =
    clauses.forall(_.evaluate(selectedFeatures))
...
class Or(val clauses: Set[SATFeatureExpr]) extends BinaryLogicConnective[Or] {
  ...
  override def create(clauses: Iterable[SATFeatureExpr]): SATFeatureExpr = FExprBuilder.createOr(clauses)
  ...
  private def combineEquiCNF(cnfchildren: Set[SATFeatureExpr]) =
    if (cnfchildren.exists(_.isInstanceOf[And])) {
      val orClauses = ArrayBuffer[SATFeatureExpr]() //list of Or expressions
      val renamedDisjunction = ArrayBuffer[SATFeatureExpr]()
      for (child <- cnfchildren) {
        child match {
          case And(innerChildren) =>
...
class Not(val expr: SATFeatureExpr) extends HashCachingFeatureExpr {
  override def calcHashCode: Int = 701 * expr.hashCode
  ...
  override def toTextExpr: String = "!" + expr.toTextExpr
  ...
  override def debug_print(ind: Int): String = indent(ind) + "!\n" + expr.debug_print(ind + 1)

  override def calcSize: Int = expr.size
...
object Not {
  def unapply(x: Not): Some[SATFeatureExpr] = Some(x.expr)
}
...
abstract class DefinedExpr extends SATFeatureExpr with SingleFeatureExpr {
  /*
   * This method is overridden by children case classes to return the name.
  ...
  override def mapDefinedExpr(f: DefinedExpr => SATFeatureExpr, cache: mutable.Map[SATFeatureExpr, SATFeatureExpr]): SATFeatureExpr = cache.getOrElseUpdate(this, f(this))

  override def calcCNF: SATFeatureExpr = this

  override def calcCNFEquiSat: SATFeatureExpr = this
}
...
object DefinedExpr {
  ...
  def checkFeatureName(name: String): Unit = assert(name != "1" && name != "0" && name != "", "invalid feature name: " + name)
}
...
class DefinedExternal(name: String) extends DefinedExpr {
  ...
  override def toString: String = "def(" + name + ")"
  ...
  override def evaluate(selectedFeatures: Set[String]): Boolean = selectedFeatures contains name
...
class DefinedMacro(val name: String, val presenceCondition: SATFeatureExpr, val expandedName: String, val presenceConditionCNF: Susp[FeatureExpr /*CNF*/ ]) extends DefinedExpr {
  ...
  override def toString: String = "macro(" + name + "=" + presenceCondition.resolveToExternal.toString + ")"

  override def satName: String = expandedName
  ...
  override def evaluate(selectedFeatures: Set[String]): Boolean = presenceCondition.evaluate(selectedFeatures)
...
object DefinedMacro {
  def unapply(x: DefinedMacro): Some[(String, SATFeatureExpr, String, Susp[FeatureExpr])] = Some((x.name, x.presenceCondition, x.expandedName, x.presenceConditionCNF))
}
...
private[sat] case class StructuralEqualityWrapper(f: SATFeatureExpr) {
  ...
  final override def hashCode: Int = f.hashCode

  final def unwrap: SATFeatureExpr = f
}
