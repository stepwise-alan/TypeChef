...
abstract class AbstractLinuxFeatureModelTest {
  ...
  val dimacsFile: URI = this.getClass.getResource("/x86.dimacs").toURI
...
class BDDLinuxFeatureModelTest extends AbstractLinuxFeatureModelTest {
  ...
  def getFeatureModel: FeatureModel = featureModel

  def getFeatureExprFactory: AbstractFeatureExprFactory = FeatureExprFactory.bdd

  @Test
  def testFeatureModelAssumptionsDimacs(): Unit = {
  ...
  @Test
  def testFeatureModelAssumptions(): Unit = {
...
class SATLinuxFeatureModelTest extends AbstractLinuxFeatureModelTest {
  val featureModel: FeatureModel = de.fosd.typechef.featureexpr.sat.SATFeatureModel.createFromDimacsFile(Source.fromURI(dimacsFile))

  def getFeatureModel: FeatureModel = featureModel

  def getFeatureExprFactory: AbstractFeatureExprFactory = FeatureExprFactory.sat
}