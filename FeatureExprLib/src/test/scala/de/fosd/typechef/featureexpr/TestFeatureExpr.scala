...
class TestFeatureExpr extends TestCase {
  ...
  @Test
  def assertSimplify(exprA: FeatureExpr, expectedResult: FeatureExpr): Unit = {
  ...
  @Test
  def testExprEvaluationSAT(): Unit = {
    testEval(FeatureExprFactory.sat)
  }

  @Test
  def testExprEvaluationBDD(): Unit = {
    testEval(FeatureExprFactory.bdd)
  }


  private def testEval(f: AbstractFeatureExprFactory): Unit = {
  ...
  private def assertEquivalent(exp: FeatureExpr, actual: FeatureExpr): Unit = assert(exp equivalentTo actual, "%s NOT equiv %s".format(exp, actual))

  @Test
  def testUniqueQuantificationBDD(): Unit = {
  ...
  @Test
  def testUniqueQuantificationSAT(): Unit = {
