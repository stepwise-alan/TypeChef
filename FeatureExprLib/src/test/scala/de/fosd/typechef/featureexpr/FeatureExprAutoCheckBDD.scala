...
object FeatureExprAutoCheckBDD extends Properties("FeatureExpr-BDD") {
  def feature(a: String): SingleFeatureExpr = createDefinedExternal(a)

  val featureNames: List[String] = List("a", "b", "c", "d", "e", "f")
  val a: SingleFeatureExpr = feature("a")
  val b: SingleFeatureExpr = feature("b")
  val c: SingleFeatureExpr = feature("c")
  val d: SingleFeatureExpr = feature("d")
  val e: SingleFeatureExpr = feature("e")
  val f: SingleFeatureExpr = feature("f")

  val genAtomicFeatureWithDeadAndBase: Gen[FeatureExpr] =
    oneOf(True :: False :: featureNames.map(feature))
  val genAtomicFeatureWithoutDeadAndBase: Gen[SingleFeatureExpr] =
    oneOf(featureNames.map(createDefinedExternal))
  ...
  private def genFeatureExpr(genAtomicFeature: Gen[FeatureExpr]) = {
  ...
  val fm: FeatureModel = BDDFeatureModel.createFromDimacsFile("FeatureExprLib/src/test/resources/small.dimacs")
