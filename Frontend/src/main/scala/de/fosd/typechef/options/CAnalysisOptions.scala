...
class CAnalysisOptions extends FeatureModelOptions with ICTypeSysOptions with ICAnalysisOptions {
  ...
  private val Sdoublefree = SecurityOption("doublefree", "Issue a warning when dynamically allocated memory is freed multiple times", dflt = false)
  private val Sxfree = SecurityOption("xfree", "Issue a warning when trying to free memory that was not allocated dynamically", dflt = false)
  private val Sunitializedmemory = SecurityOption("uninitializedmemory", "Issue a warning when the value of a non-initialized variable is used", dflt = false)
  private val Scasetermination = SecurityOption("casetermination", "Issue a warning when statements following a case block within a switch aren't terminated using a break statement", dflt = false)
  private val Sdanglingswitchcode = SecurityOption("danglingswitchcode", "Issue a warning when code in a switch statement doesn't occur within the control flow of a case or default statement", dflt = false)
  private val Scfginnonvoidfunc = SecurityOption("cfginnonvoidfunction", "Issue a warning when control flow in non-void function reaches no return statement", dflt = false)
  private val Sstdlibfuncreturn = SecurityOption("checkstdlibfuncreturn", "Issue a warning when the return value of a standard library function is not check for its error values", dflt = false)
  private val Sdeadstore = SecurityOption("deadstore", "Issue a warning when values stored to variables are never read afterwards", dflt = false)
  ...
  override def warning_implicit_coercion: Boolean = Aimplicitcoercion.isSelected
  ...
  protected override def interpretOption(c: Int, g: Getopt): Boolean = {
    ...
    if (c == 'A') {
      ...
      if (arg == "ALL") opts.foreach(_.isSelected = true)
      else {
      ...
      opts.foreach(_.isSelected = false)
    }
